...
Assume that \(\Var\{m(X)\}>0\), \(\E[Y^2]<\infty\), and \(\E[m(X)^2]<\infty\). Then
\[
...
Assume:
\begin{enumerate}[leftmargin=6mm,label=(\roman*)]
\item \textbf{Boundedness:} There exists \(C_0<\infty\) such that \(|\E[Y \mid \mhat(X)]|\le C_0\) almost surely and \(\sup_x |m_{n,\mathrm{iso}}^\star(x)| = O_p(1)\). \label{cond::bound}
\item \textbf{Controlled tails:} The conditional error \(Y-\E[Y\mid \mhat(X)]\) is sub-Gaussian or subexponential under \(P_0\). \label{cond::subgaus}
\end{enumerate}
\end{assumption}
