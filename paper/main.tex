...
\title{Better Prediction-Powered Inference via Calibration}
...
\maketitle
\begin{abstract}
We study semisupervised mean estimation with a small labeled sample, a large unlabeled sample, and an available black-box prediction model that may be miscalibrated. This setting arises in randomized trials, biobanks, and other data-rich scientific studies where gold-standard outcomes are expensive but predictions or surrogate scores are readily available. Standard prediction-powered estimators such as augmented inverse-probability weighting (AIPW) \citep{robins1994estimation} remain valid even when the prediction model is misspecified, but they can leave efficiency on the table when predictions are poorly calibrated. We propose a simple correction to this workflow: \textit{predict, calibrate, and average}. Using the labeled sample, we recalibrate the prediction model using linear or isotonic regression and then average the calibrated predictions over the pooled sample. This lightweight step is easy to add to existing pipelines, requires no retraining of the original model, preserves the simple plug-in form and validity guarantees of AIPW, and can improve efficiency relative to AIPW based on uncalibrated predictions.

We also clarify how our approach relates to recent prediction-powered inference (PPI) methods. We show that the original PPI estimator of \citet{ppi2023} is a special case of classical AIPW \citep{robins1994estimation} and can be less efficient when the prediction model is accurate, because it does not fully use labeled-sample covariate information. We further show that PPI++ of \citet{angelopoulos2023ppi} is AIPW with empirical-efficiency maximization \citep{rubin2008empirical} and is first-order equivalent to linear calibration \citep{mincer1969evaluation} and classical prognostic-score regression adjustment \citep{hansen2008prognostic}. Thus, PPI methods are best viewed as part of a broader class of familiar semiparametric and regression-adjustment methods. Consistent with this perspective, calibration-based estimators often improve on the classical PPI estimator and remain competitive with or outperform AIPW and PPI++ in simulations and real-data experiments. We implement the proposed methods in an accompanying \texttt{Python} package, \href{https://larsvanderlaan.github.io/ppi-aipw/}{\texttt{ppi\_aipw}}.
\end{abstract}
 
\section{Introduction}

We study semisupervised mean estimation with one labeled dataset \(\{(X_i, Y_i)\}_{i=1}^n\) and one unlabeled dataset \(\{\widetilde X_i\}_{i=1}^N\), where outcomes are observed only for a subset sampled completely at random. This is a classical missing-outcome problem: we observe rich covariate information for many individuals, but the outcome of interest is available only for a smaller subset. The goal is to use the larger unlabeled sample to improve estimation of a population mean while still accounting for the fact that predictions are imperfect. This setting has long been studied in missing-data and semiparametric statistics, where methods such as AIPW, TMLE, and related debiased estimators already allow flexible machine-learning predictors to be used with valid inference \citep{robins1994estimation,vanderlaanunified,van2006targeted,vanderLaanRose2011,DoubleML}. More recently, similar ideas have reappeared in the machine-learning literature under the name prediction-powered inference \citep{ppi2023,angelopoulos2023ppi,crossppi2024}. Related problems also arise in covariate adjustment for randomized trials, where machine-learning predictors trained on rich auxiliary data can improve precision in small samples \citep{hansen2008prognostic,rubin2008empirical,moore2009covariate,lin2013agnostic,schuler2022increasing,demirel2024prediction,jin2026prognostic}.


In many applications, the bottleneck is not covariates or predictions, but gold-standard outcomes. A randomized trial may have a prognostic model trained on historical data \citep{schuler2022increasing}, a biobank analysis may use a phenotype predictor built from EHR or omics measurements \citep{hong2019semisupervised,zhang2019phecap}, and a computational biology pipeline may use predictions to supplement slow or expensive experimental measurements at scale \citep{ppi2023,crossppi2024}. In each case, investigators want to use these abundant predictions to estimate a population mean or treatment-arm mean more precisely, without treating the predictions as error-free.
...
This motivates calibration. Calibration is a stronger notion of predictive reliability \citep{zadrozny2001obtaining,zadrozny2002transforming,niculescu2005predicting}: rather than correcting only the average bias \(\E[Y-m(X)]\), one uses the labeled sample to transform the score so that its numerical values better match the outcomes it is meant to predict. Ideally, one would like a recalibrated score \(m_n^\star(X)\) satisfying \(\E[Y\mid m_n^\star(X)] = m_n^\star(X)\). This immediately implies \(\E[m_n^\star(X)] = \E[Y]\), so the resulting plug-in estimator is automatically calibrated for the mean. This is attractive in practice because a useful prediction model may already be available, while a relatively small labeled sample can often recalibrate its output without retraining the underlying model \citep{roth2022uncertain}. Such recalibration can be implemented using classical post-processing methods including Platt scaling \citep{platt1999probabilistic}, linear calibration \citep{mincer1969evaluation}, histogram binning \citep{zadrozny2001obtaining}, and isotonic regression \citep{zadrozny2002transforming}. This is especially relevant for modern machine-learning predictors, such as neural networks, which are often poorly calibrated \citep{bella2010calibration,guo2017calibration,wang2023calibration}.

Our proposal is therefore simple: fit a black-box regression model, calibrate it on the labeled sample, and then average the calibrated predictions over the pooled covariate sample. This remains a simple ``predict, calibrate, and average'' procedure, but we show that it also has a useful statistical interpretation. In particular, the resulting estimator is exactly equal to an AIPW estimator built from the calibrated scores, with calibration removing the residual correction term. Calibration thus turns a simple plug-in rule into a debiased estimator with strong efficiency guarantees.


\paragraph{Contributions.}
We make three main contributions.

First, in Section~\ref{sec:calppi}, we introduce a general calibration-based framework for semisupervised mean estimation when a small labeled sample and a larger unlabeled sample are both available. The framework covers familiar post-hoc calibration methods, including linear calibration, histogram binning, and isotonic regression; see, in particular, Sections~\ref{sec:iso} and \ref{sec:linear}.

Second, we show that calibration can provide three important statistical benefits. It can improve the quality of the prediction model and thereby improve the precision of point estimates and confidence intervals. It also makes the resulting plug-in estimator automatically bias-corrected, since it is equal to an AIPW estimator. Finally, the calibrated score is optimal within the chosen class of transformations, in the sense that no further recalibration within that class can improve predictive performance.

Third, we study concrete calibration methods and develop asymptotic theory for the resulting estimators. We highlight isotonic regression as a simple default because it is lightweight, tuning-free, and learns a monotone transformation of the prediction score directly from the labeled data. In Section~\ref{sec:linear}, we also study linear calibration and show that PPI++ can be viewed as a form of linear regression adjustment using \(m(X)\) as a prognostic score, clarifying the connection between our framework and classical covariate-adjustment methods in randomized trials and missing-at-random settings. In Section~\ref{sec:theory}, we establish asymptotic normality, valid inference, and efficiency results for the isotonic-calibrated plug-in estimator under the joint two-sample model. We show that its asymptotic behavior is governed by the best monotone transformation of the prediction score, which can yield efficiency gains over AIPW based on the uncalibrated score. We also show that, after isotonic calibration, further post-processing of the fitted score yields no additional first-order improvement relative to an oracle that knows the regression of the outcome on that fitted score. When the limiting calibrated score coincides with the true outcome regression, the estimator is nonparametrically efficient.

More broadly, our calibrated plug-in estimators can also be viewed as targeted minimum loss estimators \citep{van2006targeted,vanderLaanRose2011}, in which calibration plays the role of the targeting step, and as a special case of calibrated debiased machine learning \citep{van2024automatic}.

Section~\ref{sec:setup} reviews the two-sample semisupervised model, the AIPW, PPI, and PPI++ estimators, and the main efficiency concepts used in the paper. Section~\ref{sec:exp} then presents synthetic and real-data experiments.
...
We begin with the augmented inverse-probability weighted (AIPW) estimator in the unrestricted two-sample model \citep{robins1994estimation}. It is helpful to view AIPW not as a single estimator, but as a family of estimators indexed by a scoring function \(f:\mathcal X\to\R\) with finite second moment. For any such \(f\), define
\begin{equation}
...
\Pl\{Y-f(X)\}.
\end{equation}
This estimator has two parts: a plug-in term based on the score \(f(X)\), and a residual correction based on the labeled sample. Its key property is that, for any fixed choice of \(f\), \(\widehat\psi(f)\) is unbiased in finite samples. In other words, every estimator in this class targets the correct mean. The main role of \(f\) is therefore not validity, but precision: different choices of \(f\) can lead to different large-sample variances.

To characterize the leading large-sample error in the two-sample model, we decompose the estimation error into one contribution from the labeled sample and one from the unlabeled sample. We say that \(\widehat\psi\) is asymptotically linear with influence pair \((D^L,D^U)\) if
\[
\widehat\psi-\psi_0
=
...
o_p(M^{-1/2}),
\]
where \(\E_{P_0}\{D^L(X,Y)\}=0\) and \(\E_{\Px}\{D^U(\widetilde X)\}=0\). In other words, up to a negligible remainder, the estimation error behaves like the average of observation-level contributions from the labeled and unlabeled samples. By the central limit theorem and independence of the two samples,
\[
...
\]
The influence pair therefore determines the estimator's asymptotic variance and provides a convenient way to compare efficiency across different choices of \(f\). The next result uses this representation to characterize the AIPW class.identifies the influence pair for the AIPW class.
...
For orientation, we record several familiar choices of \(f\) within the class \eqref{eq:aipw-family}. A standard choice is \(f(X)=m(X)\), where \(m(X)\) is a prediction rule, yielding the usual AIPW estimator \citep{robins1994estimation}:
\[
...
\right\},
\]
where \(\rho_n^{-1}\) is the known inverse probability of observing the outcome. Because outcome missingness is completely at random with known constant probability \(\rho_n\), this estimator is unbiased in finite samples even if \(m\) is misspecified. Thus, in the present setting, the choice of prediction rule affects efficiency but not validity, so AIPW is ``safe'' \citep{xu2025unified}. This is a special case of double robustness\footnote{In more general missing-at-random settings with unknown, covariate-dependent missingness probabilities, the analogous AIPW estimator remains doubly robust asymptotically, but it no longer enjoys the same finite-sample unbiasedness as in the present known-\(\rho_n\) setting \citep{seaman2018introduction}.} \citep{bang2005doubly}.

Another estimator in this class was proposed by \citet{ppi2023} under the name PPI,\footnote{The PPI and PPI++ estimators are algebraically equivalent to the model-assisted survey-design estimators of \citet{cassel1976some} and \citet{sarndal2003model}, respectively; see \citet{mozer2026ppi} for a review.} and we adopt that terminology here. It corresponds to the score choice
\[
...
\]
Relative to \(\widehat{\psi}_{\mathrm{AIPW}}\), \(\widehat{\psi}_{\mathrm{PPI}}\) places the entire plug-in term on the unlabeled sample and rescales the score by the factor \((1-\rho_n)^{-1}>1\). As a result, even when \(m(X)\) is reasonably accurate, this rescaling can move it farther from the true regression function \(\mu_0(X)\), which typically reduces efficiency. In particular, \(\widehat{\psi}_{\mathrm{PPI}}\) is inefficient when \(m=\mu_0\). When the labeled fraction \(\rho_n\) is small and \(m\) is inaccurate, however, AIPW need not be more efficient than PPI \citep{han2012note,rotnitzky2012improved}. This can happen when \(\Cov(m(X),Y-m(X))\) is sufficiently positive, so that the plug-in and residual-correction terms are positively correlated in a way that increases, rather than decreases, variance.

Because both \(\widehat{\psi}_{\mathrm{AIPW}}\) and \(\widehat{\psi}_{\mathrm{PPI}}\) use scores in the scaling class \(\{\lambda m(X):\lambda\in\mathbb R\}\), it is natural to choose the scaling factor data-adaptively rather than fixing it in advance. One way to do this is to minimize the estimator's empirical influence-function variance over this class:
\[
\sigma_n^2(f)
:=
...
\]
This procedure is known as empirical-efficiency maximization: one chooses \(\lambda\) to minimize \(\sigma_n^2(f)\) over the class \(\{\lambda m(X):\lambda\in\mathbb R\}\) \citep{rubin2008empirical,vanderlaanunified}. Related ideas were used by \citet{rotnitzky2012improved} to construct doubly robust estimators with improved efficiency under misspecification. The resulting estimator, called PPI++ by \citet{angelopoulos2023ppi}, is simply the AIPW estimator with an empirically rescaled prediction rule \(\hat{\lambda}m(X)\), where \(\hat{\lambda}\) estimates the population-optimal scaling coefficient
\begin{equation}
\label{opt:scale}
\lambda^\star
=
...
\end{equation}
The same idea extends beyond simple rescaling. For example, one can minimize \(\sigma_n^2(f)\) over richer classes such as $\{\lambda m(X)+\beta_{\mathrm{lin}}^\top X:
\lambda \in \mathbb R,\ \beta_{\mathrm{lin}} \in \mathbb R^d\},$
over monotone transformations of the score, or over a reproducing kernel Hilbert space (Appendix~\ref{appendix:remarks}). More generally, the empirical efficiency criterion can also be used in cross-validation to choose among candidate estimator classes \citep{rubin2008empirical}.



The best possible asymptotic variance in this model is achieved by choosing \(f(X)=\mu_0(X)\), the true regression function, although this function is unknown and must be estimated in practice. This highlights a simple point: to attain full efficiency, one generally needs to use the full covariate information \(X\), or at least a summary rich enough to recover \(\mu_0(X)\) \citep{robins1994estimation}. In practical terms, prediction scores that are closer to \(\mu_0(X)\) leave smaller residuals and therefore typically lead to greater efficiency and narrower confidence intervals. The next result formalizes this benchmark by identifying the efficient influence pair in this model, and hence the semiparametrically efficient variance bound \citep{bickel1993efficient}. Modern debiased machine learning methods pursue this same goal by using flexible black-box learners to estimate nuisance functions \citep{vanderlaanunified,moore2009covariate,moore2009increasing,vanderLaanRose2011,DoubleML}.
...
This characterization shows that the AIPW class provides the relevant first-order benchmark in this model. We now illustrate how several estimators that are not always presented in AIPW form---namely regression adjustment, balancing-weight adjustment, and TMLE---can nonetheless be viewed as finite-sample or asymptotic equivalents of AIPW estimators \citep{rotnitzky2025note,van2006targeted}, while sometimes offering better finite-sample performance \citep{porter2011relative, jin2026prognostic, van2025automatic}.
...
\hfill \newline \textbf{Regression plug-in estimators.} A particularly simple member of this first-order class is obtained through plug-in regression adjustment \citep{scharfstein1999adjusting,van2006targeted,rosenblum2009using,rotnitzky2025note}. Let
\[
...
Section~2.2 showed that AIPW provides a safe debiasing template, while the choice of score \(f(X)\) determines efficiency. We now study how to improve an available prediction score \(m(X)\) through post-processing, without retraining the original model. Using the labeled sample, we learn a transformation of \(m(X)\) within a chosen class and use the resulting calibrated score to form a plug-in estimator.

\subsection{General estimator and exact AIPW representation}

Let \(m:\mathcal X\to\mathbb R\) denote a black-box regression fit. A calibration procedure uses the labeled scores \(m(X_1),\dots,m(X_n)\) and outcomes \(Y_1,\dots,Y_n\) to fit a post-hoc map \(f_n\), and then defines the calibrated regression model
\[
m_n^\star := f_n\circ m.
\]
The fitted values on the labeled and unlabeled samples are therefore \(m_n^\star(X_i)\) for \(i=1,\dots,n\) and \(m_n^\star(\widetilde X_j)\) for \(j=1,\dots,N\). Intuitively, the map \(f_n\) updates the original score \(m(X)\) so that its numerical values better match the outcome.


Our post-processing step has three goals. First, it aims to improve the original prediction score \(m\) without retraining the underlying model. Second, it aims to remove the average residual on the labeled sample, which will yield an exact AIPW representation of the resulting plug-in estimator. Third, it seeks a calibrated score for which no further recalibration within a chosen class of transformations can improve empirical prediction.
...
To obtain a strong post-processing guarantee, we use a notion of empirical calibration that says the calibrated score is already as good as any further recalibration within a chosen class.

Let \(\mathcal F\) be a linear class of real-valued functions \(f:\mathbb R\to\mathbb R\) that contains all affine maps \(t\mapsto a+bt\), where \((a,b)\in\mathbb R^2\). We say that \(\mtilde\) is \emph{empirically calibrated over \(\mathcal F\)} if, for every \(f\in\mathcal F\),
\begin{equation}
\label{eq:cal-risk}
...
\end{equation}
where \(\Pl\) denotes the empirical mean over the labeled sample. In words, once \(\mtilde\) has been calibrated, no further transformation of \(\mtilde\) within \(\mathcal F\) can improve empirical squared-error fit on the labeled sample.

Equivalently, the first-order optimality conditions for \eqref{eq:cal-risk} are
\begin{equation}
\label{eq:cal-score}
...
f\in\mathcal F.
\end{equation}
Because \(\mathcal F\) contains the constant functions and the identity map, these conditions imply, in particular,
\[
\Pl\{Y-\mtilde(X)\}=0,
\qquad
\Pl\bigl[\mtilde(X)\{Y-\mtilde(X)\}\bigr]=0.
\]
The first identity is the empirical mean-calibration condition: the residuals of the calibrated score average to zero on the labeled sample, which gives the exact AIPW representation below. The second shows that \(\mtilde\) is already optimal along the one-dimensional scaling class \(\{\lambda \mtilde:\lambda\in\mathbb R\}\), so that no rescaling of \(\mtilde\) can improve empirical squared error. Two cases will be especially important below: full calibration, where \(\mathcal F\) is unrestricted, and linear calibration, where \(\mathcal F\) is restricted to affine maps.

 

 
The next result gives the basic structural property of calibrated plug-in estimators.
...
\end{equation}
\end{theorem}
Theorem~\ref{thm:repr} shows that empirical calibration does more than improve prediction: it gives the calibrated plug-in estimator an exact debiased representation. In particular, \(\psihat\) coincides exactly with an AIPW estimator using \(\mtilde\) as the prediction score. The same calibration conditions also yield an equivalent balancing-weight representation, showing that \(\psihat\) can be written as a weighted average of \(Y\) over the labeled sample, with the unlabeled sample entering only through the weights. This representation is useful both conceptually and practically: it makes clear how information from the unlabeled sample is incorporated, and it allows the analyst to use familiar weighting diagnostics, such as balance checks and inspection of the weight distribution, to assess the behavior of the estimator.
 

\textbf{Full versus linear calibration.} We focus on two main cases. In the nonparametric case, \(\mathcal F\) is the class of all transformations \(f:\R\to\R\), which we call \emph{full calibration}. In this case, calibration is defined relative to the richest possible post-processing class: a predictor is fully calibrated if no further transformation of it can improve prediction. In practice, however, the calibrated predictor is obtained by regressing \(Y\) on the initial score \(m(X)\) over a chosen optimization class \(\mathcal H\), which need not coincide with \(\mathcal F\). The class \(\mathcal H\) plays the role of a regularizer: without such a restriction, one could overfit the labeled sample, achieving perfect empirical fit but poor prediction and poor calibration at the population level. For example, \(\mathcal H\) may consist of piecewise constant or monotone functions, as in histogram regression or isotonic regression (Section~\ref{sec:iso}). In the parametric case, both \(\mathcal H\) and \(\mathcal F\) are restricted to affine transformations \(t\mapsto a+bt\), which we call \emph{linear calibration}. This corresponds to fitting the working linear model \(a\,m(X)+b\) for \(Y\) (Section~\ref{sec:linear}).

\textbf{Why calibration can improve efficiency.} The representation in Theorem~\ref{thm:repr} explains why calibration yields a valid debiased estimator; the remaining question is why it may also improve precision. Suppose that the calibrated score \(\mtilde\) converges to a population limit \(m_0\), where
\[
m_0(X)=g_0\{m(X)\},
\qquad
...
\]
Thus, \(m_0\) is the best predictor obtainable from the original score \(m\) within the class \(\mathcal H\). If \(\mathcal H\) consists of monotone or affine transformations, then \(m_0\) is no worse than \(m\) under squared-error risk, since the identity map belongs to both classes. When \(\mtilde\) is close to \(m_0\), \(\psihat\) behaves like the oracle AIPW estimator that uses \(m_0\). Calibration can therefore improve efficiency insofar as \(m_0\) provides a better regression adjustment than the original score \(m\).
 

\subsection{Full calibration via isotonic regression}
\label{sec:iso}

In practice, full empirical calibration can be obtained by fitting a second-stage calibrator to the labeled pairs \(\{(\mhat(X_i),Y_i)\}_{i=1}^n\). Two especially simple options are histogram binning and isotonic calibration \citep{zadrozny2001obtaining,zadrozny2002transforming,niculescu2005predicting}. Histogram binning fits a piecewise-constant map on a prespecified partition of the score \citep{gupta2021distribution}, whereas isotonic calibration estimates the best monotone transformation of the score and adaptively determines the effective bins using the pooled-adjacent-violators algorithm \citep{barlow1972isotonic}. We focus on isotonic regression because it is lightweight, tuning-free, and easy to implement using widely available software. Moreover, because the identity map is monotone, isotonic calibration can only improve the empirical mean squared error over \(\mathcal D_L\) relative to the original scores. It is therefore a natural choice in our framework, since it improves empirical fit and, as we show below, also yields the exact bias-correction and no-further-recalibration properties that motivate our approach.
...
Consequently, the representations of Theorem \ref{thm:repr} holds with \(\mathcal F\) equal to the class of all real-valued functions. In the context of treatment effect calibration, Proposition \ref{prop:iso-repr} was derived by \citet{van2023causal}.
...
\label{sec:linear}
For small labeled samples, one may prefer smoother parametric calibration rules to histogram binning or isotonic regression. Here we focus on linear calibration, which enforces that the prediction score is linearly uncorrelated with its residuals and is implemented by regressing the outcome on the prediction score \citep{mincer1969evaluation}. Relative to isotonic regression, linear calibration imposes a more restrictive parametric form, akin in spirit to Platt scaling and temperature scaling \citep{platt1999probabilistic,guo2017calibration}, but it may be more stable when the labeled sample is small. Its guarantee is correspondingly narrower: it need not correct nonlinear miscalibration, but it is optimal within the class of affine post-processing steps, namely additive shifts and rescalings of the original score.
...
The next result connects linear calibration to PPI++, a closely related estimator that chooses a rescaling of the original score by empirical efficiency maximization. In the mean-estimation problem, these two approaches are first-order equivalent, although they need not coincide exactly in finite samples. Intuitively, both are trying to make the same one-dimensional adjustment: they choose the best linear rescaling of \(m(X)\). Let \(\widehat{\psi}_{++}(\widehat{\lambda}_{++})\) denote the PPI++ estimator of \citet{angelopoulos2023ppi}, that is, the AIPW estimator based on the rescaled score \(\widehat{\lambda}_{++}m(X)\), where \(\widehat{\lambda}_{++}\) is obtained by empirical efficiency maximization over \(\{\lambda m(X):\lambda\in\R\}\) \citep{rubin2008empirical}. The key point is that \(\widehat{\lambda}_{++}\) estimates \(\lambda_0\) in \eqref{opt:scale}, which is exactly the population slope in the linear regression of \(Y\) on \(m(X)\). 
...
We now study the isotonic-calibrated plug-in estimator \(\widehat{\psi}_{\mathrm{iso}}\). We first establish its asymptotic linearity, asymptotic normality, and the validity of Wald inference. We then study its efficiency, giving conditions under which it attains the minimum asymptotic variance among regular estimators \citep{bickel1993efficient,van2000asymptotic}. We also show that isotonic calibration is already optimal at first order in a natural sense: after calibration, further post-processing of the fitted score does not improve mean squared error or the leading behavior of the resulting plug-in estimator. To keep the exposition light, we focus on the non-cross-fitted estimator and treat the initial black-box score \(\mhat\) as fixed.

\subsection{Asymptotic normality and inference}

Throughout this subsection, let \(f_0\) denote a population isotonic calibration map for the fixed score \(\mhat\), defined by
\begin{equation}
\label{eq:pop-iso}
...
The target of the asymptotic analysis is the population isotonic limit \(m_0\), which need not equal the true regression \(\muzero\). Under \cref{assump:isoreg}, the isotonic estimator attains the classical rate
\[
\|m_{n,\mathrm{iso}}^\star-m_0\|_{2,\Px}^2 = O_p(n^{-2/3});
\]
see, for example, \citet{chatterjee2015risk,yang2018contraction}.
...
The next corollary records the resulting asymptotic normality and the asymptotic validity of standard Wald confidence intervals. The limiting variance of \(\widehat{\psi}_{\mathrm{iso}}\) is
\[
\sigma_0^2
:=
...
The next corollary describes two important efficiency regimes: one in which isotonic calibration attains full semiparametric efficiency, and one in which it is optimal among estimators that use the covariates only through the one-dimensional score \(\mhat(X)\).
...
\item If \(\E_{P_0}[Y\mid \mhat(X)]\) is monotone increasing in \(\mhat(X)\), then \(m_0(X)=\E_{P_0}[Y\mid \mhat(X)]\) almost surely. Hence \(\widehat{\psi}_{\mathrm{iso}}\) is efficient for \(\psi_0\) in the reduced two-sample experiment obtained by replacing \(X\) with the scalar score \(\mhat(X)\).
\end{enumerate}
\end{corollary}


\begin{remark}[Practical takeaway]
Isotonic calibration can improve efficiency when the original score is informative but miscalibrated. The resulting estimator remains a simple plug-in estimator, but behaves as if the original score \(\mhat(X)\) were replaced by its best monotone calibration \(m_0(X)\). Because affine maps with positive slope are isotonic, it is at least as efficient as AIPW, PPI, PPI++, and linear calibration when those estimators are built from the same score and use positive scaling or slope coefficients. More generally, when \(\E_{P_0}[Y\mid \mhat(X)]\) is monotone, no regular estimator based only on the original score \(\mhat(X)\) can improve on the isotonic-calibrated estimator at first order.
\end{remark}


\begin{remark}[When full efficiency is attained]
Full efficiency requires that the calibrated target recover all regression information relevant to \(\psi_0\). This holds if \(m_0=\mu_0\) almost surely. More generally, full efficiency fails when the one-dimensional score \(\mhat(X)\) omits predictive information in \(X\) that cannot be recovered by monotone calibration.
\end{remark}

\subsection{No further first-order gain from post-processing}

We now ask whether the isotonic-calibrated score can be improved by an additional post-processing step. The key observation is that \(\widehat{\psi}_{\mathrm{iso}}\) uses the covariates only through the one-dimensional score \(m_{n,\mathrm{iso}}^\star(X)\). It is therefore natural to view the estimator as operating in the reduced two-sample experiment
\[
...
In this reduced experiment, the natural benchmark is the estimator that uses the regression of \(Y\) on the fitted isotonic score, namely \(\E_{P_0}[Y\mid m_{n,\mathrm{iso}}^\star(X)]\), in place of \(m_{n,\mathrm{iso}}^\star(X)\). The result below shows that isotonic calibration is already close enough to this benchmark that further post-processing yields no first-order improvement. In practical terms, once the score has been isotonic-calibrated, no additional recalibration of that fitted score can further improve the large-sample behavior of the resulting plug-in estimator.
...
Assumption~\ref{assump:eff-bv} is mild. It requires only that the population regression of \(Y\) on the initial score \(\mhat(X)\), viewed as a one-dimensional function of the score, have bounded total variation. In particular, it allows jumps and is weaker than differentiability or Lipschitz conditions.
