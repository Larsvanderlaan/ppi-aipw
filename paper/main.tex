...
+
\frac{1-\rhozero}{\rhozero}\Var_{P_0}\bigl\{f(X)-\mu_0(X)\bigr\}.
\end{align*}
...
\[
\frac{1-\rho_0}{\rho_0}\Var_{P_0}\bigl\{f(X)-\mu_0(X)\bigr\}.
\]
...
Standard AIPW and prediction-powered estimators treat the prediction score \(m(X)\) as fixed. However, in many applications, an available score may be useful for ranking individuals while still being poorly calibrated, in the sense that its numerical values do not accurately represent the outcome itself. For example, a score may correctly assign higher values to individuals with larger outcomes on average, yet still systematically understate or overstate the true conditional mean. Because the efficiency of the semisupervised estimator depends on the quality of the regression adjustment, such miscalibration can leave substantial efficiency gains unrealized.  



Motivated by this observation, we study a simple post-hoc strategy: use the labeled sample to calibrate \(m(X)\), and then plug the calibrated score into the semisupervised mean estimator. We call the resulting approach \emph{calibrated prediction-powered inference}. The guiding idea is that improving the score as a predictor of the outcome can yield a more efficient AIPW estimator. As we show below, this construction admits a simple plug-in form together with an exact AIPW representation. Different choices of calibration class lead to different calibrated PPI estimators, including the linear and isotonic procedures studied below.
...
\mtilde := f_n \circ m.
\]
Our estimator averages the calibrated predictions over the pooled labeled and unlabeled covariates:
\begin{equation}
\label{eq:plugin}
\psihat
:=
...
Formally, calibration seeks to transform the original score \(m(X)\) into a new score \(\mtilde(X)\) satisfying
\[
\mtilde(X)\approx \E[Y\mid \mtilde(X)].
\]
Thus, among individuals with the same predicted value, the average outcome should be close to that value. For example, in binary classification, among individuals assigned predicted probability \(0.7\), roughly \(70\%\) should have outcome \(Y=1\). Under squared error loss, calibration is also closely tied to optimal post-processing, since replacing \(\mtilde(X)\) by \(\E[Y\mid \mtilde(X)]\) cannot increase mean squared error. In our setting, this matters because a better regression adjustment can yield a more efficient semisupervised estimator. A natural target is therefore \(\E[Y\mid m(X)]\), the best predictor obtainable from the original score alone. Our aim is to post-process \(m(X)\) to move it closer to this target, without retraining the underlying model.
...
So far, calibration has been described informally as a population property: the score should be interpretable as a prediction of the outcome and should admit no further improvement by post-processing. Because the true distribution \(P_0\) is unknown, this property cannot be imposed directly. Instead, we require an empirical analogue on the labeled sample. Let \(\mathcal F\) be a linear class of maps \(f:\mathbb R\to\mathbb R\) containing the constant functions. We say that \(\mtilde\) is \textit{empirically calibrated} over \(\mathcal F\) if
\begin{equation}
\label{eq:cal-risk}
...
\end{equation}
Because \(\mathcal F\) is linear, this condition is equivalent to
\begin{equation}
\label{eq:cal-score}
...
\text{for all } f\in\mathcal F.
\end{equation}
As we show next, empirical calibration yields not only the exact AIPW representation in \eqref{eqn::aipwcal}, but also first-order equivalence to an oracle-processed AIPW estimator.


\begin{theorem}[Representation relative to the population-optimal further post-processing]
\label{thm:repr}
Let \(m_n^\star\) satisfy \eqref{eq:cal-score} over a linear class \(\mathcal F\) containing the constant functions. Define
\[
m_{0,\mathcal F}^\dagger := f_{0,\mathcal F}\circ m_n^\star, \qquad f_{0,\mathcal F}\in \arg\min_{f\in\mathcal F}
E_0\!\left[(Y-f(m_n^\star(X)))^2\right].
\]
Then
\begin{align*}
\psihat
...
&\qquad
+ (1-\rhon)(\Pu-\Pl)\!\left[m_n^\star-m_{0,\mathcal F}^\dagger\right].
\end{align*}
\end{theorem}
 


Theorem~\ref{thm:repr} is our key structural result. It shows that \(\psihat\) differs from the AIPW estimator based on \(m_{0,\mathcal F}^\dagger\), the population-optimal post-processing of \(m_n^\star\) within \(\mathcal F\), only through the remainder
\[
(1-\rhon)(\Pu-\Pl)\!\left[m_n^\star-m_{0,\mathcal F}^\dagger\right].
\]
Under \cref{eq:cal-score}, one can often show that \(m_n^\star\) is close to \(m_{0,\mathcal F}^\dagger\) in mean squared error, so that this remainder is higher order. Consequently, \(\psihat\) has the same first-order behavior as the corresponding oracle AIPW estimator. Thus, for purposes of inference, empirical calibration makes the fitted score behave as though it had already been optimally post-processed within \(\mathcal F\). Empirical calibration also implies a balancing-weights representation for \(\psihat\) of the form \(\Pl\{\hat w_{\mathrm{bal}}(X)Y\}\); see Appendix~\ref{app:repr}. We formalize this for isotonic calibration in Section~\ref{sec:theory}.
...
Calibration can improve efficiency because it improves the regression adjustment appearing in the exact AIPW representation of \eqref{eqn::aipwcal}. If \(f_n\) converges to a population minimizer \(g_0\in\mathcal H\), then the post-processed score \(\mtilde=f_n\circ m\) converges to
\[
m_0(X):=g_0\{m(X)\},
\qquad
g_0\in\arg\min_{g\in\mathcal H}
E_0\!\left[\{Y-g(m(X))\}^2\right].
\]
Thus, \(m_0\) is the best regression adjustment obtainable from the original score \(m\) using transformations in \(\mathcal H\). By Proposition~\ref{prop:aipw-class}, the resulting AIPW estimator has asymptotic variance no larger than that of any estimator based on \(\theta\circ m\) with \(\theta\in\mathcal H\). In particular, if the identity map belongs to \(\mathcal H\), then calibration cannot do worse than using the original score \(m\). If \(\mathcal H\) is rich enough to contain the oracle map \(t\mapsto E_0[Y\mid m(X)=t]\), then \(m_0(X)=E_0[Y\mid m(X)]\), and the resulting estimator is semiparametrically efficient in the reduced model based on the score \(m(X)\) \citep{ji2025predictions}.

\textbf{Full versus linear calibration.}
We focus on two main cases. In \emph{full calibration}, the target class \(\mathcal F\) is unrestricted, so that, at the population level, no further transformation of the calibrated score can improve mean squared prediction error. In practice, however, the fitted map \(f_n\) is learned over a restricted optimization class \(\mathcal H\), which acts as a regularizer. Important examples include piecewise-constant and monotone classes, corresponding to histogram and isotonic calibration; see Section~\ref{sec:iso}. In \emph{linear calibration}, both \(\mathcal H\) and \(\mathcal F\) are restricted to affine maps \(t\mapsto a+bt\), corresponding to least-squares fitting of the working model \(a+b\,m(X)\); see Section~\ref{sec:linear}.
...
Within this reduced experiment, a natural benchmark is the oracle regression function $\E_0\!\left[ Y \mid m_{n,\mathrm{iso}}^\star(X) \right],$ where the conditioning is understood to be conditional on the training data used to construct \(m_{n,\mathrm{iso}}^\star\). The result below shows that no additional post-processing of the fitted score can improve the first-order performance of \(\widehat{\psi}_{\mathrm{iso}}\) relative to this benchmark.
...
Theorem~\ref{prop:eif-red} also shows that the asymptotic variance of \(\widehat{\psi}_{\mathrm{iso}}\) matches that of the oracle estimator \(\widehat{\psi}_{\mathrm{eff}}\), which uses the oracle score \(\widehat{m}_0(X):=\E_0[Y \mid m_{n,\mathrm{iso}}^\star(X)]\). Equivalently, within the reduced experiment based on \(m_{n,\mathrm{iso}}^\star(X)\), \(\widehat{m}_0(X)\) is the oracle post-processing that minimizes the variance of the AIPW influence function. Moreover, Theorem~\ref{thm:al} combined with Proposition \ref{prop:aipw-class} implies that \(\widehat{\psi}_{\mathrm{iso}}\) achieves the optimal limiting variance within the class of monotone post-processings, and is therefore at least as efficient as AIPW based on the original score \(m(X)\). Hence, \(\widehat{\psi}_{\mathrm{iso}}\) \emph{functional-estimation-calibeats} \(\widehat{\psi}_{\mathrm{AIPW}}\) and \(\widehat{\psi}_{\mathrm{PPI}}\).
...
\caption{Synthetic simulation study with a poorly calibrated score. The top row shows the balanced case \(N/n=1\), and the bottom row shows the large-unlabeled case \(N/n=16\). The four columns report absolute bias, empirical Monte Carlo standard deviation, coverage, and relative efficiency versus PPI.}
...
\caption{PPE-only numerical summary at \(n\in\{100,400\}\), chosen to contrast the moderate-label and larger-label regimes. ``Human label savings'' and ``Correctness label savings'' report the estimated fraction of labels saved relative to labeled-only for the same variance; Avg.\ cov.\ averages coverage across the two PPE tracks.}
