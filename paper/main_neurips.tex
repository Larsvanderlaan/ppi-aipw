...
\maketitle
\begin{abstract}
We study semisupervised mean estimation with a small labeled sample, a large unlabeled sample, and an available black-box prediction model that may be miscalibrated. This setting arises in randomized trials, biobanks, and other data-rich studies where gold-standard outcomes are expensive but surrogate predictions are readily available. Standard prediction-powered estimators such as augmented inverse-probability weighting (AIPW) \citep{robins1994estimation} remain valid under prediction-model misspecification, but they can leave efficiency on the table when the prediction score is poorly calibrated.

We propose a simple post-hoc calibration correction: using the labeled sample, we recalibrate the prediction score and then average the calibrated predictions over the pooled sample. This procedure, which we call \emph{Calibrated Prediction-Powered Inference}, requires no retraining of the original model and can improve efficiency relative to AIPW based on the original uncalibrated score. We study both linear and isotonic calibration. For isotonic calibration, we establish a first-order \emph{calibeating} guarantee: the calibrated estimator can improve on the original score and on simpler post-processing rules, and no further post-processing of the fitted isotonic score yields additional first-order improvement. For linear calibration, we show first-order equivalence to PPI++ and hence the same asymptotic efficiency.

We also clarify that recent PPI methods are particular AIPW-based procedures within a broader semiparametric regression-adjustment framework. In simulations and benchmark experiments, our calibrated estimators often improve on PPI and remain competitive with, or outperform, AIPW and PPI++.
\end{abstract}
 
\section{Introduction}

We study semisupervised mean estimation with one labeled dataset \(\{(X_i,Y_i)\}_{i=1}^n\) and one unlabeled dataset \(\{\widetilde X_i\}_{i=1}^N\), where outcomes are observed only for a subset sampled completely at random. This is a classical missing-outcome problem: rich covariate information is available for many individuals, but the outcome of interest is observed only for a smaller subset. Such settings arise when gold-standard outcomes are expensive to collect but prediction scores are readily available, for example in randomized trials with prognostic models trained on historical data \citep{schuler2022increasing}, biobank analyses using phenotype predictors built from EHR or omics measurements \citep{hong2019semisupervised,zhang2019phecap}, and computational biology pipelines that supplement slow or costly experimental measurements with model-based predictions \citep{ppi2023,crossppi2024}. The goal is to use the larger unlabeled sample to improve estimation of a population mean while accounting for prediction error. This setting has long been studied in missing-data and semiparametric statistics, where methods such as AIPW, TMLE, and related debiased estimators allow flexible machine-learning predictors to be used with valid inference \citep{robins1994estimation,vanderlaanunified,van2006targeted,vanderLaanRose2011,DoubleML}. More recently, related ideas have reappeared in the machine-learning literature under the name prediction-powered inference \citep{ppi2023,angelopoulos2023ppi,crossppi2024}. Closely related questions also arise in covariate adjustment for randomized trials, where machine-learning predictors trained on auxiliary data can improve precision in small samples \citep{hansen2008prognostic,rubin2008empirical,moore2009covariate,lin2013agnostic,schuler2022increasing,demirel2024prediction,jin2026prognostic}.

A standard approach is to use the labeled sample to debias a plug-in estimator based on a prediction model. If \(m(X)\) is a black-box prediction score, the plug-in estimator averages \(m(X)\) over the pooled covariate sample, while the labeled sample is used to estimate the residual correction term \(\E\{Y-m(X)\}\). The augmented inverse-probability weighted (AIPW) estimator \citep{robins1994estimation} averages \(m(X)\) over both labeled and unlabeled samples and then adds the average residual over the labeled sample, whereas the PPI estimator \citep{ppi2023} averages \(m(X)\) only over the unlabeled covariates before applying the same residual correction. PPI can be less efficient because it discards labeled-covariate information. Thus, while these methods protect against bias from imperfect prediction models, they typically treat the prediction score itself as fixed. In practice, however, useful black-box prediction scores are often systematically too high, too low, or too extreme. Such misalignment may arise because the model was trained on a different population, or because of temporal drift, distribution shift, or regularization bias.

This motivates post-hoc calibration of the prediction score. Rather than correcting only the marginal bias \(\E\{Y-m(X)\}\), one can use the labeled sample to transform the score so that its numerical values better align with the outcomes they are meant to predict \citep{zadrozny2001obtaining,zadrozny2002transforming,niculescu2005predicting}. An ideal target is a calibrated score \(m_n^\star(X)\) satisfying \(\E[Y\mid m_n^\star(X)] = m_n^\star(X)\). Under this condition, \(\E[m_n^\star(X)] = \E[Y]\), so the corresponding plug-in estimator is automatically unbiased for the population mean. This is attractive because a useful prediction model may already be available, while a relatively small labeled sample can recalibrate its output without retraining \citep{roth2022uncertain}. Classical post-processing methods include Platt scaling \citep{platt1999probabilistic}, linear calibration \citep{mincer1969evaluation}, histogram binning \citep{zadrozny2001obtaining}, and isotonic regression \citep{zadrozny2002transforming}. Such calibration is especially relevant for modern machine-learning predictors, which are often informative yet systematically miscalibrated \citep{bella2010calibration,guo2017calibration,wang2023calibration}.

Our proposal is therefore simple: fit a black-box regression model, calibrate it on the labeled sample, and average the calibrated predictions over the pooled covariate sample. We call this workflow \emph{Calibrated Prediction-Powered Inference}. This remains a simple ``predict, calibrate, and average'' procedure, but we show that it also admits an exact debiased interpretation. In particular, the resulting estimator is exactly equal to the AIPW estimator based on the calibrated score, with the residual correction encoded by the calibration step itself. Because calibration can improve the score, it can also improve efficiency relative to AIPW based on the uncalibrated model. Calibration therefore converts a simple plug-in rule into a debiased estimator with attractive efficiency properties.

\paragraph{Contributions.}
We make three main contributions. First, we introduce a general \emph{Calibrated Prediction-Powered Inference} framework for semisupervised mean estimation, covering familiar post-hoc methods such as linear and isotonic calibration. Second, we show that calibration provides three gains: it can improve the prediction score and therefore precision; it makes the pooled plug-in estimator automatically debiased because it is exactly equal to AIPW with the calibrated score; and it can produce a fitted score that cannot be improved by further post-processing within the relevant class, a form of \emph{calibeating} in the sense of \citet{foster2023calibeating}. Third, we develop asymptotic theory for these estimators. In particular, isotonic-calibrated PPI gives a lightweight default with valid inference, efficiency gains under miscalibration, and no further first-order gain from post-processing, while linearly calibrated PPI is first-order equivalent to PPI++ and related to classical prognostic-score regression adjustment.
...
\textbf{Semiparametric statistics and missing data.} This paper builds on foundational ideas from the missing-data and semiparametric literatures. Rubin formalized missing at random as a key identification condition for recovering full-data targets from partially observed outcomes \citep{rubin1976}, and semiparametric efficiency theory then characterized efficient influence functions, regular asymptotically linear estimators, and one-step debiased procedures \citep{robins1994estimation,robins1995analysis,robins1995semiparametric,vanderlaanunified,tsiatis2006semiparametric,bickel1993efficient}. In missing-data and causal-inference problems, these ideas led to influence-function-based estimating equations and the efficiency framework used throughout this paper.

\textbf{Debiased machine learning and targeted learning.} A large body of work combines this semiparametric framework with machine-learning-based nuisance estimation under the labels of targeted learning and debiased machine learning \citep{van2006targeted,vanderLaanRose2011,DoubleML,chernozhukov2022automatic}. In the present setting, these methods already imply that flexible black-box predictors can be used with valid inference once the missingness probability is known, which is exactly the lens through which we interpret score-based semisupervised estimators.

\textbf{Semisupervised inference and PPI.} Related work studies semisupervised or surrogate-assisted estimation when the primary outcome is observed only on a limited subset \citep{pepe1992inference,pepe1994auxiliary,chen2000miscellanea,chen2008improving,cheng2021robust,ji2025predictions,kallus2025role}. More recent machine-learning papers popularized the label \emph{prediction-powered inference} (PPI) for the semisupervised mean-estimation setting \citep{ppi2023,angelopoulos2023ppi,crossppi2024}. These papers helped popularize the setting, but methodologically PPI estimators are best understood as instances or variants of existing semiparametric procedures. For example, PPI++ is AIPW with empirical-efficiency maximization \citep{rubin2008empirical,vanderlaanunified}. MEC \citep{lee2026mec} instead uses generalized entropy weights to balance the prediction score, which is closer to the balancing-weight representation of our linearly calibrated estimator. Closely related questions have long been studied in randomized trials \citep{hansen2008prognostic,rosenblum2009using,moore2009covariate,moore2009increasing,schuler2022increasing,hojbjerre2025powering}. Here we emphasize the corresponding regression-adjustment view, implemented by ordinary linear regression of \(Y\) on the score.

\textbf{From missing data to causal inference.} From the perspective of estimation and efficiency theory, semisupervised mean estimation with missing-at-random outcomes is statistically equivalent to randomized experiments and related sampling problems once the missingness indicator is reinterpreted as a treatment or sampling indicator \citep{rubin1978bayesian,vanderlaanunified,licausal,mozer2026ppi}. This equivalence allows tools from covariate adjustment, causal inference, and model-assisted estimation to transfer directly to the semisupervised setting.

\textbf{Calibration.} A final relevant thread concerns calibration. Classical post-processing methods such as Platt scaling, linear calibration, isotonic regression, and histogram binning improve predictive reliability without retraining the underlying model \citep{platt1999probabilistic,cox1958two,gupta2023online,mincer1969evaluation,zadrozny2001obtaining,zadrozny2002transforming,niculescu2005predicting,gupta2021distribution}. Most closely related for our purposes is recent work connecting calibration to debiased estimation of statistical functionals \citep{van2024automatic,van2023adaptive,van2025semiparametric}.
...
\textbf{Full versus linear calibration.} We focus on two main cases. In \emph{full calibration}, \(\mathcal F\) is the class of all real-valued transformations, while \(\mathcal H\) is the fitting class used to learn \(f_n\). This distinction matters because \(\mathcal H\) serves as a regularizer: without such a restriction, one could overfit the labeled sample and achieve perfect empirical fit with poor population prediction. In practice, \(\mathcal H\) may consist of piecewise-constant or monotone maps, as in histogram and isotonic calibration. In \emph{linear calibration}, by contrast, \(\mathcal H=\mathcal F\) is the affine class \(t\mapsto a+bt\), so both fitting and no-further-calibration are assessed within the same working linear model for \(Y\) given \(m(X)\).
...
\]
Thus, \(m_0\) is the best predictor obtainable from the original score \(m\) using post-processing maps in the class \(\mathcal H\). When \(\mtilde\) is close to \(m_0\), \(\psihat\) behaves like the AIPW estimator that uses \(m_0\). Proposition~\ref{prop:aipw-class} therefore implies that this limiting estimator has asymptotic variance no larger than that of the AIPW estimator based on any \(\theta\circ m\) with \(\theta\in\mathcal H\). In particular, if \(\mathcal H\) contains the identity map, as it does for affine or monotone transformations, then \(m\) itself is among the admissible post-processings. It follows that the estimator based on \(m_0\) is asymptotically at least as efficient as the AIPW estimator based on the original score \(m\). Calibration can therefore improve efficiency insofar as \(m_0\) provides a better regression adjustment than \(m\).
...
\label{sec:linear}
For smaller labeled samples, one may prefer the lower-dimensional affine case. We therefore focus on linear calibration \citep{mincer1969evaluation}, where the fitting and calibration classes coincide as the affine maps. It regresses the outcome on the original score and therefore corrects mean bias, scaling, and location shift inside that class.
...
Linear calibration can thus be viewed as classical prognostic-score adjustment in semisupervised form \citep{hansen2008prognostic,rosenblum2009using,lin2013agnostic,schuler2022increasing}. Appendix~\ref{appendix:remarks} discusses richer smooth and generalized-linear-model extensions, including Platt scaling and Venn--Abers shrinkage.
\subsection{Cross-Fitting and Cross-Calibration}
\label{sec:crossfit}

In the main exposition, we assume for simplicity that the initial black-box predictor is fixed. In practice, however, it may be trained on the labeled sample itself, in which case the same labeled observations are used both to fit the predictor and to calibrate its scores. A cross-fitted version replaces \(m\) by out-of-fold prediction scores, fits the calibrator on those out-of-fold labeled scores, and then evaluates the calibrated score on the unlabeled sample. This is the natural DML-style implementation when one wishes to reduce overfitting from using the same labeled data both to estimate the initial regression and to calibrate it for inference, so we defer the algorithmic details to the appendix discussion.

\section{Theory for isotonic-calibrated PPI}
\label{sec:theory}

We now study the theoretical basis for \emph{calibeating} via isotonic calibration. Our goal is to understand when post-processing an available score by isotonic regression can improve on the original score and on simpler calibration rules. We establish asymptotic linearity, asymptotic normality, valid Wald inference, and efficiency results, and then show that no further post-processing of the fitted isotonic score yields additional first-order gain. To keep the exposition light, we focus on the non-cross-fitted estimator and treat the initial black-box score \(\mhat\) as fixed.
...
Under \cref{assump:isoreg}, the isotonic estimator attains the classical rate
\[
...
\]
where \(z(S):=\mathrm{logit}\{\mu_0(S)\}\). This preserves ranking information while introducing nonlinear distortion on the probability scale. We compare \textsc{Naive}, PPI, AIPW, clipped PPI++, unclipped \textsc{AIPW-EM}, \textsc{LinearCal}, \textsc{MonoSpline}, \textsc{IsoCal}, and \textsc{AutoCal}; PPI++ uses the official \texttt{ppi\_py} clipping rule, while \textsc{AIPW-EM} does not. The labeled sample size ranges over \(n\in\{50,100,200,400,800,1200,2400\}\), with \(N/n\in\{1,16\}\), and each setting uses 500 Monte Carlo repetitions.
...
\caption{Binary simulation with a miscalibrated score. Columns report bias, Monte Carlo standard deviation, coverage, and relative efficiency versus PPI for \(N/n\in\{1,16\}\).}
\label{fig:simulation-rmse}
\end{figure}

\Cref{fig:simulation-rmse} shows that all score-based AIPW estimators are essentially unbiased, so the differences are mainly in variance. Under miscalibration, especially when \(N/n=16\), calibration improves efficiency. In balanced designs PPI++ and \textsc{AIPW-EM} track \textsc{LinearCal}; when the unlabeled sample is large, \textsc{AIPW-EM} stays near \textsc{LinearCal} while clipped PPI++ falls back toward PPI. \textsc{AutoCal} stays near the best fixed option.


\subsection{Empirical Illustration}
\label{sec:empirical}

We evaluate the proposed calibration-based estimators on the mean-estimation benchmarks used in the original experiments of \citet{ppi2023,angelopoulos2023ppi}, as provided in the \texttt{ppi\_py} package. We use the same datasets and labeled-sample-size grids, repeatedly split each fully labeled benchmark into labeled and unlabeled subsets, evaluate all estimators on the same split, treat the full-sample mean as ground truth, and average over 500 random splits. We report \texttt{forest}, \texttt{galaxies}, and \texttt{census\_income}.

Baselines are the labeled-only interval, imputation on binary tasks, PPI, AIPW, clipped PPI++, and unclipped \textsc{AIPW-EM}. The main-text figure focuses on PPI, AIPW, PPI++, \textsc{AIPW-EM}, \texttt{AutoCal}, and \texttt{LinearCal}; Appendix~\ref{app:empirical} gives the richer calibration comparison.


\begin{figure}[htb]
\centering
\includegraphics[width=\textwidth]{assets/fig_paper_main_grid.pdf}
\caption{Benchmark summary for PPI, AIPW, clipped PPI++, \textsc{AIPW-EM}, \texttt{AutoCal}, and \texttt{LinearCal}. Panels report normalized MSE, relative efficiency, and coverage versus PPI.}
\label{fig:ppi-main}
\end{figure}

\Cref{fig:ppi-main} suggests a simple practical picture. AIPW is the natural estimator based on the original score, and it typically matches or outperforms PPI; when the unlabeled sample is enormous, they nearly coincide, as in \texttt{census\_income}. Post-hoc calibration is most useful when the score is meaningfully miscalibrated and the labeled sample is large enough to estimate the second-stage calibrator stably, with \texttt{LinearCal} strongest on \texttt{census\_income}. The added \textsc{AIPW-EM} baseline sharpens the comparison with PPI++: once the efficiency-maximization step is left unclipped, it tracks \texttt{LinearCal} much more closely, whereas official PPI++ clips the scaling coefficient to \([0,1]\), motivated by shrinkage estimators, and can therefore fall back toward PPI when the variance-minimizing rescaling would exceed \(1\). Overall, the practical comparison is that AIPW is the right raw-score baseline, and when miscalibration is substantial the gains come from calibration or unclipped empirical-efficiency maximization rather than from clipped PPI++.


\section{Conclusion}

We introduced \emph{Calibrated Prediction-Powered Inference} for semisupervised mean estimation. Calibration can improve efficiency without retraining the original model, isotonic calibration can \emph{calibeat} the original score and simpler post-processing rules at first order, and linearly calibrated PPI is first-order equivalent to PPI++ and related to classical prognostic-score regression adjustment.
...
This characterization shows that the AIPW class is the relevant first-order benchmark in this model. Appendix~\ref{section::plugin} illustrates that several estimators not always written in AIPW form---including regression adjustment, balancing-weight adjustment, and TMLE---can nonetheless be viewed as finite-sample or asymptotic equivalents of AIPW estimators \citep{rotnitzky2025note,van2006targeted}, while sometimes offering better finite-sample performance \citep{porter2011relative,jin2026prognostic,van2025automatic}.
