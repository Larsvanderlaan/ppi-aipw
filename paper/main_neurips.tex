...
\maketitle
\begin{abstract}
We study semisupervised mean estimation with a small labeled sample, a large unlabeled sample, and an available black-box prediction model that may be miscalibrated. A standard approach in this setting is augmented inverse-probability weighting (AIPW) \citep{robins1994estimation}, which is robust to prediction-model misspecification but may be inefficient when the prediction score is poorly calibrated. We propose \emph{Calibrated Prediction-Powered Inference}: post-hoc calibrate the prediction score on the labeled sample and then average the calibrated predictions over the pooled sample. This simple post-hoc step requires no retraining and can improve efficiency relative to using the original score directly. We study both linear and isotonic calibration. For isotonic calibration, we prove a first-order \emph{calibeating} guarantee: isotonic post-processing can improve on the original score and on simpler post-processing rules in both predictive accuracy and estimator efficiency, and no further post-processing of the fitted isotonic score yields additional first-order improvement. For linear calibration, we show first-order equivalence to PPI++. We also show that the original PPI estimator is a special case of AIPW and can be inefficient when the prediction model is accurate, while PPI++ is AIPW with empirical-efficiency maximization \citep{rubin2008empirical}. In simulations and real-data experiments, our calibrated estimators often outperform PPI and remain competitive with, or outperform, AIPW and PPI++. We provide an accompanying \texttt{Python} package, \href{https://larsvanderlaan.github.io/ppi-aipw/}{\texttt{ppi\_aipw}}.
\end{abstract}
 
\section{Introduction}

We study semisupervised mean estimation with one labeled dataset \(\{(X_i,Y_i)\}_{i=1}^n\) and one unlabeled dataset \(\{\widetilde X_i\}_{i=1}^N\), where outcomes are observed only for a subset sampled completely at random. This is a classical missing-outcome problem: rich covariate information is available for many individuals, but the outcome of interest is observed only for a smaller subset. Such settings arise when gold-standard outcomes are expensive but prediction scores are readily available, for example in randomized trials with prognostic models trained on historical data \citep{schuler2022increasing}, in biobank analyses using phenotype predictors built from EHR or omics data \citep{hong2019semisupervised,zhang2019phecap}, and in other data-rich pipelines that supplement costly measurements with model-based predictions \citep{ppi2023,crossppi2024}. This problem has long been studied in missing-data and semiparametric statistics, where methods such as AIPW, TMLE, and related debiased estimators allow flexible machine-learning predictors to be used with valid inference \citep{robins1994estimation,vanderlaanunified,van2006targeted,vanderLaanRose2011,DoubleML}. More recently, related ideas reappeared in the machine-learning literature under the name prediction-powered inference \citep{ppi2023,angelopoulos2023ppi,crossppi2024} and in covariate adjustment for randomized trials \citep{hansen2008prognostic,rubin2008empirical,moore2009covariate,lin2013agnostic,schuler2022increasing,demirel2024prediction,jin2026prognostic}.

A standard approach is to use the labeled sample to debias a plug-in estimator based on a prediction model. If \(m(X)\) is a black-box prediction score, the plug-in estimator averages \(m(X)\) over the pooled covariate sample, while the labeled sample estimates the residual correction term \(\E\{Y-m(X)\}\). The AIPW estimator \citep{robins1994estimation} averages \(m(X)\) over both labeled and unlabeled samples and then adds the average residual over the labeled sample, whereas the PPI estimator \citep{ppi2023} averages \(m(X)\) only over the unlabeled covariates before applying the same residual correction. PPI can therefore be less efficient because it discards labeled-covariate information. These methods protect against bias from imperfect prediction models, but they usually treat the prediction score itself as fixed. In practice, however, useful black-box prediction scores are often systematically too high, too low, or too extreme because of population mismatch, temporal drift, distribution shift, or regularization bias.

This motivates post-hoc calibration of the prediction score. Rather than correcting only the marginal bias \(\E\{Y-m(X)\}\), one can use the labeled sample to transform the score so that its numerical values better align with the outcomes they are meant to predict \citep{zadrozny2001obtaining,zadrozny2002transforming,niculescu2005predicting}. An ideal target is a calibrated score \(m_n^\star(X)\) satisfying \(\E[Y\mid m_n^\star(X)] = m_n^\star(X)\), in which case \(\E[m_n^\star(X)] = \E[Y]\) and the corresponding plug-in estimator is automatically unbiased for the population mean. This is attractive because a useful prediction model may already be available, while a relatively small labeled sample can calibrate its output without retraining \citep{roth2022uncertain}. Classical post-processing methods include Platt scaling, linear calibration, histogram binning, and isotonic regression \citep{platt1999probabilistic,mincer1969evaluation,zadrozny2001obtaining,zadrozny2002transforming}. Such calibration is especially relevant for modern machine-learning predictors, which are often informative yet systematically miscalibrated \citep{bella2010calibration,guo2017calibration,wang2023calibration}.

An early grounded toy example makes the mechanism concrete. We take a public LLM-evaluation benchmark with human correctness labels, focus on its MATH slice, and ask how accurately one can estimate average human correctness from a small labeled sample plus many unlabeled evaluator scores. The raw evaluator margin has real signal but is poorly scaled for the human labels, so using it directly in PPI yields only modest gains. A one-dimensional recalibration fit on the labeled pairs restores the probability scale, sharply reduces held-out squared error, and yields materially shorter confidence intervals for the mean. In this sense, calibration improves label efficiency: achieving the same uncertainty with the raw score or with labeled-only estimation would require more human labels. Figure~\ref{fig:toy-calibration-ppi} visualizes this mechanism in the resulting one-mean toy.

\begin{figure}[tb]
\centering
\includegraphics[width=\textwidth]{assets/fig_toy_calibration_ppi.pdf}
\caption{Grounded one-mean toy derived from the MATH slice of a public LLM-evaluation benchmark with human correctness labels, using ArmoRM as the automatic evaluator score. Left: the raw evaluator margin has clear signal about correctness, but it is not on the right numerical scale. Middle: a Platt map fit on 100 labeled examples restores the probability scale and lowers held-out squared error by about \(58\%\). Right: in Monte Carlo with \(N=2000\) unlabeled scores, calibrated PPI yields about \(17\%\) shorter \(90\%\) Wald intervals than labeled-only estimation at \(n=25\) and about \(15\%\) shorter intervals at \(n=50\), while raw PPI provides only modest gains.}
\label{fig:toy-calibration-ppi}
\end{figure}

Our proposal is simple: fit a black-box regression model, calibrate it on the labeled sample, and average the calibrated predictions over the pooled covariate sample. We call this workflow \emph{Calibrated Prediction-Powered Inference}. It remains a simple ``predict, calibrate, and average'' procedure, but it also admits an exact debiased interpretation: the resulting estimator is exactly equal to the AIPW estimator based on the calibrated score, with the residual correction encoded by the calibration step itself. Because calibration can improve the score, it can also improve efficiency relative to AIPW based on the uncalibrated model.

\paragraph{Contributions.}
We make three main contributions. First, we study \emph{calibrated prediction-powered inference} for semisupervised mean estimation, covering familiar post-hoc methods such as linear and isotonic calibration. Second, we show that calibration can improve the prediction score and therefore precision while also making the pooled plug-in estimator automatically debiased because it is exactly equal to AIPW with the calibrated score. Third, we develop asymptotic theory: isotonic-calibrated PPI gives a lightweight default with valid inference, efficiency gains under miscalibration, and no further first-order gain from post-processing, while linearly calibrated PPI is first-order equivalent to PPI++ and related to classical prognostic-score regression adjustment.

\subsection{Related work}
Although PPI has been presented as a recent development, the underlying problem and many of its estimators sit inside a broader literature on semiparametric debiasing and regression adjustment.

\textbf{Semiparametric statistics and missing data.} Rubin formalized missing at random as a key identification condition for recovering full-data targets from partially observed outcomes \citep{rubin1976}, and semiparametric efficiency theory then characterized efficient influence functions, regular asymptotically linear estimators, and one-step debiased procedures \citep{robins1994estimation,robins1995analysis,robins1995semiparametric,vanderlaanunified,tsiatis2006semiparametric,bickel1993efficient}. This yielded the influence-function and efficiency framework used throughout the paper.

\textbf{Debiased machine learning and targeted learning.} A large body of work combines this semiparametric framework with machine-learning-based nuisance estimation under the labels of targeted learning and debiased machine learning \citep{van2006targeted,vanderLaanRose2011,DoubleML,chernozhukov2022automatic}. In the present setting, these methods already imply that flexible black-box predictors can be used with valid inference once the missingness probability is known.

\textbf{Semisupervised inference and PPI.} Related work studies semisupervised or surrogate-assisted estimation when the primary outcome is observed only on a limited subset \citep{pepe1992inference,pepe1994auxiliary,chen2000miscellanea,chen2008improving,cheng2021robust,ji2025predictions,kallus2025role}. More recent machine-learning papers popularized the label \emph{prediction-powered inference} (PPI) for this semisupervised mean-estimation setting \citep{ppi2023,angelopoulos2023ppi,crossppi2024}. Methodologically, however, PPI estimators are best understood as semiparametric variants: PPI++ is AIPW with empirical-efficiency maximization \citep{rubin2008empirical,vanderlaanunified}, MEC \citep{lee2026mec} uses generalized entropy weights to balance the prediction score, and closely related questions have long been studied in randomized trials \citep{hansen2008prognostic,rosenblum2009using,moore2009covariate,moore2009increasing,schuler2022increasing,hojbjerre2025powering}. Here we emphasize the corresponding regression-adjustment view.

\paragraph{Calibrated DML and Recalibrated PPI.}
In our mean-estimation setting, the closest precursor is the calibrated debiased machine-learning framework of \citet{van2024automatic}, which studies calibration of the outcome regression in doubly robust inference. Our problem can be viewed as a semisupervised special case of that calibrated-DML perspective, which also motivates the term \emph{calibrated PPI}. A closely related recent paper is \citet{ji2025predictions}, who study recalibrated prediction-powered inference for general estimating equations. In the mean-estimation setting, their method effectively learns the oracle correction to the initial prediction score that gives the most efficient prediction-powered estimator. In the squared-loss case, this is equivalent to learning the oracle conditional-mean recalibration of the initial score, so the two approaches are closely related. The difference is in emphasis and theory. \citet{ji2025predictions} focus on learning the oracle correction for efficiency. By contrast, we focus on calibrating the score itself and on what can be guaranteed from simple post-hoc calibration rules, even without consistently learning the oracle target. This yields lightweight plug-in methods that can be added to an existing prediction pipeline without retraining, together with calibration-specific results: an exact AIPW interpretation, first-order equivalence between linear calibration and PPI++, and a first-order calibeating guarantee for isotonic calibration.

\textbf{From missing data to causal inference.} From the perspective of estimation and efficiency theory, semisupervised mean estimation with missing-at-random outcomes is statistically equivalent to randomized experiments and related sampling problems once the missingness indicator is reinterpreted as a treatment or sampling indicator \citep{rubin1978bayesian,vanderlaanunified,licausal,mozer2026ppi}. This lets tools from covariate adjustment, causal inference, and model-assisted estimation transfer directly.

\textbf{Calibration.} Classical post-processing methods such as Platt scaling, linear calibration, isotonic regression, and histogram binning improve predictive reliability without retraining the underlying model \citep{platt1999probabilistic,cox1958two,gupta2023online,mincer1969evaluation,zadrozny2001obtaining,zadrozny2002transforming,niculescu2005predicting,gupta2021distribution}. Recent applications also include value prediction in offline reinforcement learning \citep{van2025bellman,van2025semiparametric}.
...
\]
The labeled sample consists of i.i.d.\ draws from a distribution \(P_0\) on \(\mathcal X\times\mathbb R\), while the unlabeled sample consists of i.i.d.\ draws from the distribution \(\Px\) of \(X\) under \(P_0\). Our target is the population mean \(\psi_0:=\E_{P_0}[Y]\), and we write \(\muzero(x):=\E_{P_0}[Y\mid X=x]\).
...
\]
For readability, we will sometimes write \(\Pl\{a(X,Y)\}\) instead of \(\Pl a\), and for functions of \(X\) alone we write \(\Pl\{b(X)\}=n^{-1}\sum_{i=1}^n b(X_i)\). Let \(M:=n+N\) and \(\rho_n:=n/M\); pooled score averages are written as \(\rho_n \Pl f + (1-\rho_n)\Pu f\).
...
\Pl\{Y-f(X)\}.
\end{equation}
This estimator combines a plug-in term that imputes \(Y\) from \(f(X)\) with a residual correction from the labeled sample. For any fixed \(f\), \(\widehat\psi(f)\) is unbiased in finite samples for \(\psi_0\). Thus, the choice of \(f\) affects efficiency through variance, but not validity. This makes AIPW a safe estimator \citep{xu2025unified} and a particularly simple instance of double robustness \citep{bang2005doubly}; it is the semisupervised version of the broader covariate-adjustment logic used in randomized trials \citep{moore2009covariate,moore2009increasing,schuler2022increasing,hojbjerre2025powering}.

\textbf{Asymptotic linearity and efficiency.} Appendix~\ref{app:semicolon} gives the full influence-pair characterization of this class and the efficient influence pair in the unrestricted two-sample model. For the main paper, the key consequence is that the asymptotic variance of \(\sqrt{M}\{\widehat\psi(f)-\psi_0\}\) can be written as
\[
\mathrm{AVar}\!\left\{\sqrt{M}\,(\widehat\psi(f)-\psi_0)\right\}
=
V_{\mathrm{eff}}
+
\frac{1-\rho_0}{\rho_0}\E_{P_0}\!\bigl[(\tilde f(X)-\mu_0(X))^2\bigr],
\]
where \(\tilde f(X):=f(X)-\E_{\Px}\{f(X)\}+\psi_0\) is the recentered score and \(V_{\mathrm{eff}}\) is the semiparametric efficiency bound \citep{bickel1993efficient}. Thus, within the AIPW class, efficiency is governed by the mean-squared error of the recentered score \(\tilde f\) relative to \(\mu_0(X)\), and the variance is minimized when \(\tilde f=\mu_0\), in particular at \(f=\mu_0\).

\textbf{AIPW, PPI, and empirical-efficiency maximization.} The usual AIPW estimator corresponds to \(f(X)=m(X)\), where \(m(X)\) is a prediction score. The original PPI estimator of \citet{ppi2023} corresponds to the scaled score choice \(f(X)=m(X)/(1-\rho_n)\), so it is another score choice in the same AIPW class. Relative to AIPW, PPI places the plug-in term entirely on the unlabeled sample and therefore discards labeled-sample covariate information, so it can be less efficient when the original score is already informative.

Because both AIPW and PPI lie in the scaling class \(\{\lambda m(X):\lambda\in\mathbb R\}\), it is natural to choose \(\lambda\) data-adaptively rather than fixing it in advance. This yields the estimator called PPI++ by \citep{angelopoulos2023ppi}: the AIPW estimator with the empirically rescaled score \(\hat{\lambda}m(X)\). Here \(\hat{\lambda}\) is chosen by minimizing the empirical variance criterion over \(f(X)=\lambda m(X)\), a procedure known as empirical-efficiency maximization \citep{rubin2008empirical,vanderlaanunified}, and it targets
\begin{equation}
\label{opt:scale}
\lambda^\star
=
...
\end{equation}
The same principle extends beyond simple rescaling: one can minimize the same criterion over richer classes, such as \(\{\lambda m(X)+\beta^\top X\}\), monotone score transformations, or an RKHS, and can use it in cross-validation to choose among candidate estimator classes \citep{rubin2008empirical}; see Appendix~\ref{appendix:remarks}. For the main paper, the point is simple: AIPW remains safe across the whole class, and our proposal is to improve the score by calibration rather than to change the underlying validity mechanism.
...
Section~\ref{sec:setup} showed that AIPW is safe while the choice of score \(f(X)\) determines efficiency. We now introduce calibrated prediction-powered inference, which improves an available prediction score \(m(X)\) through post-processing rather than retraining. Using the labeled sample, we learn a transformation of \(m(X)\), average the calibrated predictions over the pooled sample, and obtain an estimator that has a simple plug-in form but is exactly AIPW.

\subsection{General estimator and exact AIPW representation}

Let \(m:\mathcal X\to\mathbb R\) denote a black-box regression model. A calibration procedure uses the labeled scores \(m(X_1),\dots,m(X_n)\) and outcomes \(Y_1,\dots,Y_n\) to fit a post-hoc map \(f_n\), and then defines the calibrated regression model
\[
m_n^\star := f_n\circ m.
\]
The resulting predictions on the labeled and unlabeled samples are \(m_n^\star(X_i)\) for \(i=1,\dots,n\) and \(m_n^\star(\widetilde X_j)\) for \(j=1,\dots,N\). Intuitively, the map \(f_n\) updates the original score \(m(X)\) so that its numerical values better align with the outcome.

This post-processing step has two main goals. First, it aims to improve the original prediction score \(m\) through a simple transformation, without retraining the underlying model, ideally by improving calibration without sacrificing predictive performance. Second, it is designed so that the resulting plug-in estimator is automatically unbiased, without requiring an explicit residual correction.
...
\end{equation}
Equivalently, \(\psihat=\rhon \Pl\{\mtilde(X)\} + (1-\rhon)\Pu\{\mtilde(\widetilde X)\}\).

To distinguish how the post-processing map is fit from what property the resulting score satisfies, we introduce two function classes. Let \(\mathcal H\) be an optimization class of real-valued maps \(h:\mathbb R\to\mathbb R\), and let \(\mathcal F\) be a linear calibration class of real-valued maps \(f:\mathbb R\to\mathbb R\) that contains the constant functions and the identity map. We fit the post-processing map \(f_n\) by empirical risk minimization over \(\mathcal H\), in the sense that
\[
f_n \in \arg\min_{h\in\mathcal H}
\Pl\bigl[(Y-h(m(X)))^2\bigr].
\]
We then ask what property the resulting post-processed score \(\mtilde:=f_n\circ m\) satisfies relative to the calibration class \(\mathcal F\).

We say that \(\mtilde\) is \emph{empirically calibrated over \(\mathcal F\)} if, for every \(f\in\mathcal F\),
\begin{equation}
\label{eq:cal-risk}
...
\]
The first identity makes the AIPW residual term vanish exactly on the labeled sample.

Thus a calibration method is naturally described by a pair \((\mathcal H,\mathcal F)\): linear and fixed-bin histogram calibration have \(\mathcal H=\mathcal F\), whereas isotonic regression uses a monotone \(\mathcal H\) but, by Proposition~\ref{prop:iso-repr}, yields a much larger \(\mathcal F\).

The next result gives the basic structural property of calibrated PPI estimators.

\begin{theorem}[Exact AIPW representation under empirical calibration]
\label{thm:repr}
Suppose the calibrated score satisfies \cref{eq:cal-score}, where \(\mathcal F\) contains all affine maps. Then, for any \(\what\in\mathcal F\),
\begin{equation}
...
\end{equation}
\end{theorem}
Theorem~\ref{thm:repr} turns the pooled plug-in rule into exact AIPW with the calibrated score and, when \eqref{eq:F-balance} holds, into an equivalent balancing-weight estimator.

\textbf{Full versus linear calibration.} In \emph{full calibration}, \(\mathcal F\) is the class of all real-valued transformations, while \(\mathcal H\) is the fitting class used to learn \(f_n\). This distinction matters because \(\mathcal H\) regularizes the fit: without it, one could overfit the labeled sample and achieve perfect empirical fit with poor population prediction. In practice, \(\mathcal H\) may consist of piecewise-constant or monotone maps, as in histogram and isotonic calibration. In \emph{linear calibration}, by contrast, \(\mathcal H=\mathcal F\) is the affine class \(t\mapsto a+bt\).

\textbf{Why calibration can improve efficiency.} Suppose the calibrated score \(\mtilde\) converges to a population limit \(m_0\), where
\[
m_0(X)=g_0\{m(X)\},
\qquad
...
\]
Thus, \(m_0\) is the best predictor obtainable from the original score \(m\) using post-processing maps in \(\mathcal H\). When \(\mtilde\) is close to \(m_0\), \(\psihat\) behaves like the AIPW estimator that uses \(m_0\). Proposition~\ref{prop:aipw-class} therefore implies that this limiting estimator has asymptotic variance no larger than that of the AIPW estimator based on any \(\theta\circ m\) with \(\theta\in\mathcal H\). In particular, if \(\mathcal H\) contains the identity map, then the estimator based on \(m_0\) is asymptotically at least as efficient as the AIPW estimator based on the original score \(m\).
 

\subsection{Isotonic-calibrated PPI}
\label{sec:iso}

We now turn to isotonic regression, the main nonparametric second-stage calibrator. Fitted on the labeled pairs \(\{(\mhat(X_i),Y_i)\}_{i=1}^n\), it is tuning-free, easy to compute, and cannot worsen empirical squared error relative to the original score \citep{zadrozny2002transforming,niculescu2005predicting,barlow1972isotonic}.
...
Thus isotonic regression is the main example in which a restricted monotone fitting class \(\mathcal H\) yields a much larger calibration class \(\mathcal F\). Consequently, Theorem~\ref{thm:repr} applies with \(\mathcal F\) equal to the class of all real-valued functions. Additional implementation details appear in Appendices~\ref{appendix:remarks} and \ref{appendix:code}.
...
\label{sec:linear}
For smaller labeled samples, one may prefer the lower-dimensional affine case. We therefore focus on linear calibration \citep{mincer1969evaluation}, where the fitting and calibration classes coincide as the affine maps. It regresses the outcome on the original score and corrects mean bias, scaling, and location shift within that class.
...
\right\}.
\end{equation}
Because the affine class includes the constant function, the least-squares fit automatically enforces empirical mean calibration. As a result, the linearly calibrated plug-in estimator admits an exact AIPW representation.
...
The next result connects linear calibration to PPI++, a closely related estimator that chooses a rescaling of the original score by empirical efficiency maximization. In the mean-estimation problem, these two approaches are first-order equivalent, although they need not coincide exactly in finite samples. Intuitively, both are trying to make the same one-dimensional adjustment: they choose the best linear rescaling of \(m(X)\). Let \(\widehat{\psi}_{++}(\widehat{\lambda}_{++})\) denote the PPI++ estimator of \citet{angelopoulos2023ppi}, that is, the AIPW estimator based on the rescaled score \(\widehat{\lambda}_{++}m(X)\), where \(\widehat{\lambda}_{++}\) is obtained by empirical efficiency maximization over \(\{\lambda m(X):\lambda\in\R\}\) \citep{rubin2008empirical,vanderlaanunified}. The key point is that \(\widehat{\lambda}_{++}\) estimates \(\lambda_0\) in \eqref{opt:scale}, which is exactly the population slope in the linear regression of \(Y\) on \(m(X)\). 
 
\begin{proposition}[PPI++ and Linear Calibration]
...
Linear calibration can thus be viewed as classical prognostic-score adjustment in semisupervised form \citep{hansen2008prognostic,rosenblum2009using,lin2013agnostic,schuler2022increasing}. Richer smooth and generalized-linear-model extensions, including Platt scaling, Venn--Abers shrinkage, and cross-fitted calibration, are discussed in Appendix~\ref{appendix:remarks}; see in particular Appendix~\ref{sec:crossfit}.

\section{Theory for isotonic-calibrated PPI}
\label{sec:theory}

We now study the theoretical basis for \emph{calibeating} via isotonic calibration. We establish asymptotic linearity, asymptotic normality, valid Wald inference, and efficiency results, and then show that no further post-processing of the fitted isotonic score yields additional first-order gain. To keep the exposition light, we focus on the non-cross-fitted estimator and treat the initial black-box score \(\mhat\) as fixed.
...
Under \cref{assump:isoreg}, the isotonic estimator attains the classical rate \(\|m_{n,\mathrm{iso}}^\star-m_0\|_{2,\Px}^2 = O_p(n^{-2/3})\); see \citet{chatterjee2015risk,yang2018contraction}.
...
\label{cor:normal}
Under the conditions of \cref{thm:al}, assume that
\[
\sigma_0^2
:=
...
Across all experiments we use the same comparison hierarchy. The labeled-only estimator (reported as \textsc{Naive} in the simulation) is the safety baseline. AIPW is the main raw-score baseline, with PPI as the weaker comparator. PPI++ and \textsc{AIPW-EM} are the efficiency-rescaling competitors, and \texttt{LinearCal}, \texttt{AutoCal}, and related calibration rules are the proposed fix when the score is miscalibrated. The three subsections below therefore move from a controlled mechanism check, to reproduced benchmark evidence, to a practical LLM-evaluation setting.

\subsection{Simulation study}
\label{sec:simulation}

We first study a controlled binary-outcome simulation designed to isolate score miscalibration. Let \(S\sim N(0,1)\), define
\[
\mu_0(S)=\{1+\exp(-5S)\}^{-1},
\]
...
\]
where \(z(S):=\mathrm{logit}\{\mu_0(S)\}\). We compare \textsc{Naive}, PPI, AIPW, clipped PPI++, unclipped \textsc{AIPW-EM}, and the calibration-based estimators \textsc{LinearCal}, \textsc{MonoSpline}, \textsc{IsoCal}, and \textsc{AutoCal}. The labeled sample size ranges over \(n\in\{50,100,200,400,800,1200,2400\}\), with \(N/n\in\{1,16\}\), and each setting uses 500 repetitions.
...
\caption{Binary simulation with a miscalibrated score. Columns report bias, standard deviation, coverage, and relative efficiency versus PPI for \(N/n\in\{1,16\}\).}
\label{fig:simulation-rmse}
\end{figure}

\Cref{fig:simulation-rmse} isolates the mechanism. All score-based AIPW-type estimators are essentially unbiased here, so the comparison is mainly about efficiency. AIPW consistently improves on PPI among the raw-score methods. When the unlabeled sample is large, unclipped \textsc{AIPW-EM} remains close to \texttt{LinearCal}, whereas clipped PPI++ is pulled back toward PPI once the variance-minimizing rescaling exceeds \(1\). Under genuine miscalibration, the gains therefore come from calibration, with \texttt{AutoCal} staying near the best fixed rule.


\subsection{Empirical Illustration}
\label{sec:empirical}

We next evaluate the same estimators on the original mean-estimation benchmarks of \citet{ppi2023,angelopoulos2023ppi}, as provided in the \texttt{ppi\_py} package. Using the same datasets and labeled-sample-size grids, we repeatedly split each fully labeled benchmark into labeled and unlabeled subsets, evaluate all estimators on the same split, treat the full-sample mean as ground truth, and average over 200 random splits. We report \texttt{forest}, \texttt{galaxies}, and \texttt{census\_income}. The main-text figure focuses on PPI, AIPW, PPI++, \textsc{AIPW-EM}, \texttt{AutoCal}, and \texttt{LinearCal}; Appendix~\ref{app:empirical} gives the richer calibration comparison.


\begin{figure}[htb]
\centering
\includegraphics[width=\textwidth]{assets/fig_paper_main_grid.pdf}
\caption{Benchmark summary for the reproduced \texttt{ppi\_py} datasets. Panels report normalized MSE, relative efficiency, and coverage.}
\label{fig:ppi-main}
\end{figure}

\Cref{fig:ppi-main} shows the same pattern on the reproduced benchmarks. AIPW is the stronger raw-score baseline, with PPI competitive only when the unlabeled sample is extremely large. The official \texttt{ppi\_py} implementation clips the PPI++ scaling coefficient to \([0,1]\), so when the variance-minimizing rescaling exceeds \(1\), clipped PPI++ is pulled back toward PPI whereas unclipped \textsc{AIPW-EM} tracks \texttt{LinearCal} much more closely. Across the 30 dataset-size cells in \Cref{fig:ppi-main}, \textsc{AIPW-EM} has lower MSE than clipped PPI++ in 29 and stays nearly tied with \texttt{LinearCal}. Post-hoc calibration is most useful when the score is meaningfully miscalibrated and the labeled sample is large enough to fit the second-stage calibrator stably, with \texttt{LinearCal} strongest on \texttt{census\_income} and \texttt{AutoCal} close to the best fixed option.

\subsection{LLM evaluation benchmark}
\label{sec:llm-benchmark}

We now turn to a practical LLM-evaluation setting where the same comparison hierarchy is applied to real public model outputs, public evaluator scores, and public labels. The first track uses PPE Human Preference V1, where the estimand for each target model is its human win share across prompt--response comparisons, with ties counted as half-wins. The second uses a PPE correctness suite over MMLU-Pro, MATH, GPQA, IFEval, and MBPP+, where each unit compares a sampled conflict pair from the same generator model and the estimand is the correctness rate of the first answer in the pair. In both tracks the prompts, outputs, judgments, and evaluator scores are real; only the labeled/unlabeled split is rerandomized. Across 200 repeated splits we compare labeled-only estimation against raw PPI/AIPW, the efficiency-maximized variants, and the calibration-based estimators from our main experiments. The main-text summary in \Cref{fig:llm-eval-main,tab:llm-eval-summary} therefore focuses on the completed PPE tracks only, while we leave RewardBench to a separate robustness analysis.
...
\caption{PPE-only numerical summary at \(n\in\{100,400\}\). Human and correctness columns report MSE and implied label savings relative to labeled-only; Avg.\ cov.\ averages coverage across the two PPE tracks.}
\label{tab:llm-eval-summary}
\end{table}
}{}

Two messages stand out. First, raw proxy use can fail badly. Raw AIPW is again the stronger raw-score baseline, but even AIPW can be dramatically worse than labeled-only when the reward-model margin is badly scaled. On PPE Human at \(n=100\), raw PPI and raw AIPW have MSE \(0.495\) and \(0.440\), compared with \(0.001425\) for labeled-only and \(0.001391\) for \texttt{LinearCal}; on PPE Correctness at \(n=100\), the corresponding MSEs are \(0.534\), \(0.353\), \(0.002010\), and \(0.001813\). These estimators remain valid first-order corrections, but the extra proxy term can inflate variance enough that labeled-only is far safer.

Second, calibration makes the same evaluator scores practically useful. \texttt{LinearCal} is the strongest and most stable method overall, with \texttt{AutoCal} close behind. On PPE Human, \texttt{LinearCal} is best from \(n=50\) onward, improving on labeled-only from \(0.001425\) to \(0.001391\) at \(n=100\) and from \(0.000286\) to \(0.000278\) at \(n=400\), corresponding to modest \(2.5\%\) to \(3.1\%\) label savings. On PPE Correctness, the gains are clearer: \texttt{LinearCal} is best at every budget, improving from \(0.002010\) to \(0.001813\) at \(n=100\) and from \(0.000248\) to \(0.000222\) at \(n=400\), with about \(7.5\%\) to \(10.5\%\) label savings. \texttt{MonoSpline} substantially improves over raw PPI/AIPW and is usually competitive with PPI++ and \textsc{AIPW-EM}, but it is slightly less stable than \texttt{LinearCal} and usually a bit worse than \texttt{AutoCal}.

Third, the evaluator-specific appendix breakdown (\Cref{fig:llm-eval-by-evaluator}) shows that the most extreme raw-proxy failures are driven by evaluators such as Skywork and Athene, whereas ArmoRM is much more benign. Finally, calibration also helps on the downstream ranking task induced by PPE Human. Raw PPI/AIPW recover the model ordering poorly, whereas calibrated and efficiency-maximized methods recover it well. At \(n=200\), \texttt{AutoCal} attains the highest average Spearman correlation, about \(0.905\); at \(n=400\), \texttt{LinearCal} gives the best overall ranking quality, with Spearman about \(0.941\) and top-1 identification about \(0.646\). We therefore view calibration not as a marginal efficiency tweak, but as the step that makes cheap public LLM evaluators safe enough to use in practice; see Appendix~\ref{app:llm-ranking}.
...
Although PPI has been presented as a recent development, the underlying problem and many of its estimators are special cases of a broader literature on semiparametric debiasing and regression adjustment, including work on missing data, causal inference, and debiased machine learning.
...
\section{Semiparametric background and equivalence material}
\label{app:semicolon}
...
A key insight of Proposition~\ref{prop:aipw-class} is that the limiting variance of the AIPW estimator depends on the score function \(f\) only through the mean squared error of its recentered version \(\tilde f\), namely \(\E_{P_0}\!\bigl[(\tilde f(X)-\mu_0(X))^2\bigr]\). Thus, within the AIPW class, efficiency is maximized by choosing \(f\) so that \(\tilde f\) approximates \(\mu_0(X)\) as well as possible, with equality when \(\tilde f=\mu_0\), in particular at \(f=\mu_0\). This shows that full efficiency generally requires using the full covariate information \(X\), or at least a summary rich enough to recover \(\mu_0(X)\) \citep{robins1995semiparametric}. In practice, this motivates taking \(f\) to be an estimated prediction score \(m(X)\), learned using flexible black-box methods.

\textbf{AIPW, PPI, and empirical-efficiency maximization.}
We now record several familiar choices of \(f\) within the class \eqref{eq:aipw-family}. The most common choice is \(f(X)=m(X)\), where \(m(X)\) is a prediction score, yielding the usual augmented inverse-probability weighted (AIPW) estimator \citep{robins1994estimation}:
\[
...
\frac{\Cov\bigl(Y,m(X)\bigr)}{\Var\bigl(m(X)\bigr)}.
\end{equation}
The same principle extends beyond simple rescaling. For example, one can minimize \(\sigma_n^2(f)\) over richer classes such as $\{\lambda m(X)+\beta_{\mathrm{lin}}^\top X:
...
\subsection{Additional background on semiparametric statistics}
...
\]
because the augmentation term \(\Pl[Y-m_n(X)]\) is zero by the least-squares normal equations.
Thus, when \(m_n\) converges to the best linear predictor of \(Y\) given \(m(X)\), this plug-in estimator is typically at least as efficient as \(\widehat{\psi}_{\mathrm{AIPW}}\) and \(\widehat{\psi}_{\mathrm{PPI}}\), since it corresponds to an influence function with smaller or equal variance.
...
Following \citet{rubin2008empirical}, the empirical variance criterion from Section~\ref{sec:effreview},
\[
\sigma_n^2(f)
:=
\rho_n \Pl\left[\left\{f(X)-\widehat{\psi}(f)+\rho_n^{-1}(Y-f(X))\right\}^2\right]
...
Isotonic calibration can improve efficiency when the original score is informative but miscalibrated. The resulting estimator behaves as if \(\mhat(X)\) were replaced by its best monotone calibration \(m_0(X)\). Because the isotonic class contains the identity map and all affine maps with positive slope, the isotonic-calibrated estimator has limiting variance no larger than that of any estimator that is first-order equivalent to AIPW with score \(\theta\{\mhat(X)\}\) for such a transformation \(\theta\). This includes AIPW and PPI based on \(\mhat(X)\), as well as PPI++, and linear calibration whenever the corresponding scaling or slope coefficient is nonnegative. Thus isotonic calibration is asymptotically at least as efficient, at first order, as these methods, and can be strictly more efficient when the best monotone calibration is genuinely nonlinear. More generally, if \(\E_{P_0}[Y\mid \mhat(X)]\) is monotone, then no regular estimator based only on \(\mhat(X)\) can improve on the isotonic-calibrated estimator at first order.
\end{remark}
...
\]
defines an unbiased AIPW estimating equation for \(\theta_0\).

Moreover, let \(\hat\theta\) be a regular \(\sqrt n\)-consistent solution of this equation, and assume that \(\theta\mapsto\E[\psi(O;\theta,m,\pi_0)]\) is differentiable at \(\theta_0\) with derivative \(A_0\), and that
\[
...
&=
\E\left[
m(X,\theta)+\frac{\E(R\mid X,Y)}{\pi_0(X)}\{\varphi(Y,\theta)-m(X,\theta)\}
\right]\\
&=
\E\left[
m(X,\theta)+\frac{\E(R\mid X)}{\pi_0(X)}\{\varphi(Y,\theta)-m(X,\theta)\}
\right]\\
&=
\E\bigl[\varphi(Y,\theta)\bigr],
\end{aligned}
\]
where the third line uses \(R\perp Y\mid X\). Evaluating at \(\theta=\theta_0\) gives
\[
\E\{\psi(O;\theta_0,m,\pi_0)\}
=
\E\bigl[\varphi(Y,\theta_0)\bigr]
=
0.
\]
...
Let
\[
\Psi_m(\theta):=\E\{\psi(O;\theta,m,\pi_0)\}.
\]
The previous calculation shows that \(\Psi_m(\theta)=\E[\varphi(Y,\theta)]\) for every \(\theta\), so
\[
...
\]
with \(D_m\) given in \cref{eq:general-if}. This proves the influence-function formula.
...
\[
\E\!\left[\left(1-\frac{R}{\pi_0(X)}\right)\delta_m(X)\,\middle|\,X\right]=0.
\]
Also, using \(R\perp Y\mid X\) and \(\E[\varphi(Y,\theta_0)-m_0(X,\theta_0)\mid X]=0\),
\[
\E\!\left[
\left.
\frac{R}{\pi_0(X)}
...
\]
Applying \cref{thm:general-moment-aipw} with covariate \(S\) and observation probability \(\pi_0(S)\equiv\rho_0\) gives the displayed reduced AIPW class and the efficient choice
\[
m_0(S,\theta_0)=\E[\varphi(Y,\theta_0)\mid S].
\]

At the sample level, the two-sample implementation replaces the population labeling probability \(\rho_0\) by the known design fraction \(\rho_n=n/(n+N)\). To rewrite the empirical equation in the two-sample notation, note that
\[
\mathbb P_M\{m(S,\theta)\}
=
\rho_n \mathbb P_n^L\{m(S,\theta)\}
...
&\quad +(1-\rho_n)\Pu\{\mhat(\widetilde X)+\hat a_{\mathrm{const}}\}\\
&=
\rho_n \Pl\{\mhat(X)\}
+(1-\rho_n)\Pu\{\mhat(\widetilde X)\}\\
&\quad +\hat a_{\mathrm{const}}\\
&=
\rho_n \Pl\{\mhat(X)\}
+(1-\rho_n)\Pu\{\mhat(\widetilde X)\}\\
&\quad +\Pl\{Y-\mhat(X)\}\\
...
\[
\begin{aligned}
\rhozero \Var\{D_f^L(X,Y)\}
+(1-\rhozero)\Var\{D_f^U(\widetilde X)\}
&=
\Var\{\mu_0(X)\} \\
&\quad +
\rhozero^{-1}\E_{P_0}\!\bigl[\Var(Y\mid X)\bigr] \\
...
\[
\begin{aligned}
(\Pl-\Px)\{m_{n,\mathrm{iso}}^\star(X)-m_0(X)\}
&=
O_p(n^{-2/3}), \\
(\Pu-\Px)\{m_{n,\mathrm{iso}}^\star(\widetilde X)-m_0(\widetilde X)\}
&=
O_p\bigl(n^{-1/3}N^{-1/2}\bigr).
\end{aligned}
\]
...
\label{app:empirical}
This appendix records the benchmark construction used in \cref{sec:empirical}. The main-text benchmark figure isolates three headline contrasts: PPI versus AIPW as raw-score baselines, clipped PPI++ versus \textsc{AIPW-EM} as efficiency-maximization variants, and \texttt{LinearCal}/\texttt{AutoCal} as the main calibration-based competitors.
...
The estimator set mirrors the original PPI comparison wherever possible. In the paper-facing summaries, we report the labeled-only estimator, the imputation benchmark on binary-outcome tasks, PPI computed using the official \texttt{ppi\_py} implementation, the classical AIPW estimator based on the same prediction score, clipped PPI++, and the unclipped efficiency-maximized benchmark \textsc{AIPW-EM}. We then add five calibration-oriented comparators: linear calibration; smooth monotone spline calibration; Platt scaling on the binary-outcome benchmarks; isotonic calibration with a fixed minimum bin size of 10; and an adaptive \texttt{AutoCal} selector that chooses among AIPW, linear calibration, monotone spline calibration, and isotonic calibration by cross-validated empirical efficiency.

For the binary-outcome benchmarks, we also report Venn--Abers shrinkage. Platt scaling and Venn--Abers are omitted on \texttt{census\_income} because that outcome is not binary. Including both PPI++ and \textsc{AIPW-EM} lets us separate the underlying efficiency-maximization idea from the specific \texttt{ppi\_py} clipping rule for the tuning coefficient, while \cref{fig:ppi-calibration-appendix} isolates the fixed-calibration comparison among PPI, AIPW, \texttt{LinearCal}, \texttt{MonoSpline}, and \texttt{IsoCal}.

For the fixed calibration-based plug-in estimators, we fit the calibrator on the labeled pairs \(\{(\mhat(X_i),Y_i)\}_{i=1}^n\), transform both the labeled and unlabeled scores, and then compute the pooled plug-in mean together with the same semisupervised influence-style Wald interval used throughout the benchmark code. AIPW uses the same score but averages it over the pooled covariate sample before adding the labeled residual correction, whereas PPI uses only the unlabeled score average. In theory this retains less labeled-sample score information, though in the finite-sample benchmark results the empirical variance gap is often small and not uniform across all datasets. PPI++ and \textsc{AIPW-EM} both apply one-dimensional empirical efficiency maximization to the same score, but PPI++ inherits the official \texttt{ppi\_py} clipping rule \(\lambda\in[0,1]\) whereas \textsc{AIPW-EM} does not.

Linear calibration regresses \(Y\) on an intercept and the score, then clips the fitted values to the range of predicted outcomes observed in the labeled data to prevent extrapolation on the unlabeled data. \texttt{MonoSpline} instead fits a smooth nondecreasing spline of \(Y\) on the score, using monotonicity constraints together with a mild roughness penalty, and then applies the same clipped pooled plug-in construction. Platt scaling fits a logistic regression of the binary outcome on a stabilized logit transform of the score, while the fixed-bin isotonic estimator fits a stepwise monotone calibration map and also clips to the labeled outcome range.

The \texttt{AutoCal} estimator is a selector rather than a new calibrator: in the experiments it compares AIPW, linear calibration, monotone spline calibration, and fixed-bin isotonic calibration using 20-fold cross-validation and the estimated influence-function variance as the selection criterion. To keep this step inexpensive when the unlabeled sample is very large, the foldwise criterion is evaluated on an unlabeled subsample of size \(\min(N,10n)\). The reported summaries are Monte Carlo averages of bias, empirical variance, MSE, interval coverage, and relative efficiency versus PPI across the repeated random splits. The complete numerical outputs, including dataset-level tables and per-metric plots, are generated automatically by the accompanying reproduction pipeline.

We also retain a supplementary screening sweep for the earlier score-decile gap construction. The screening compares the three public PPE-correctness evaluators (ArmoRM, Athene, Skywork) with the three reproduced tabular PPI benchmarks, always using the same top-versus-bottom-decile construction and the same small-sample calibration workflow. Table~\ref{tab:toy-anchor-screening} helps explain why that decile-gap construction is less attractive for the introduction: the classic tabular benchmarks show much smaller incremental gains, ArmoRM is comparatively mild, and Skywork is too pathological.

\begin{table}[ht]
\centering
\small
\IfFileExists{assets/table_toy_anchor_screening.tex}{
\resizebox{0.95\textwidth}{!}{\input{assets/table_toy_anchor_screening.tex}}
}{
\fbox{\parbox{0.92\textwidth}{Grounded-toy screening table not found. Rebuild the paper assets to refresh it.}}
}
\caption{Supplementary screening sweep for the earlier score-decile gap construction. Each row defines arms \(A\) and \(B\) as the top and bottom raw-score deciles for that real candidate, reports the resulting human means and raw-score means, and summarizes the held-out squared-error drop from one-dimensional recalibration. The bolded row marks the strongest candidate under that decile-gap screening rule.}
\label{tab:toy-anchor-screening}
\end{table}
...
\caption{Calibration-focused appendix comparison across the reproduced PPI benchmarks, restricted to PPI, AIPW, \texttt{LinearCal}, \texttt{MonoSpline}, and \texttt{IsoCal}. The panels report normalized MSE relative to PPI, relative efficiency versus PPI, and coverage across labeled-sample-size regimes. This figure isolates the fixed-calibration comparison omitted from the main-text benchmark figure for readability.}
...
To preserve double-blind review, repository and documentation links are omitted from the submission PDF. This appendix instead gives a minimal anonymous implementation sketch that can be bundled with any supplementary artifact.

Below we also record the three main algorithmic summaries from the full paper, using the notation of the main text.
...
def isoreg_with_xgboost(x, y, max_depth=15, min_child_weight=20, weights=None):
    ...
    x : array-like, shape (n,) or (n, 1)
        Prediction score used for calibration. In the PPI application this is
        usually the score m(X), so x is typically one-dimensional.
    ...
    -------
    predict_fn : callable
        Function mapping new x values to calibrated scores f(x).
    """
    x = np.asarray(x)
...
\item \textbf{Claims.} \textbf{Yes.} The abstract and introduction state the paper's main claims, and the paper distinguishes between exact representation results, asymptotic theory, and empirical findings; see Sections~\ref{sec:setup}--\ref{sec:exp}.

\item \textbf{Limitations.} \textbf{Yes.} The conclusion and theory sections state the paper's main scope limitations: the strongest guarantees use the known-\(\rho_n\) two-sample design and a stable second-stage calibrator; see Section~6 and Appendix~B.

\item \textbf{Theory, assumptions, and proofs.} \textbf{Yes.} The main assumptions are stated in Sections~\ref{sec:setup} and~\ref{sec:theory}, with complete proofs collected in Appendices~\ref{app:repr}, \ref{sec:ppi++}, \ref{app:eif}, \ref{app:al}, and~\ref{app:eff}.

\item \textbf{Experimental results.} \textbf{Yes.} The main experimental takeaways appear in Section~\ref{sec:exp}, with benchmark construction and additional diagnostics deferred to Appendix~\ref{app:empirical}.

\item \textbf{Open access to data and code.} \textbf{No.} This anonymous submission does not include a public repository or packaged artifact; Appendix~\ref{appendix:code} provides representative implementation sketches, and benchmark sources are cited in the text.

\item \textbf{Experimental setting and details.} \textbf{Yes.} The simulation design, benchmark protocol, estimator set, and evaluation summaries are described in Section~\ref{sec:exp} and Appendix~\ref{app:empirical}.

\item \textbf{Experimental statistical significance.} \textbf{Yes.} The experiments report Monte Carlo variability, interval coverage, and efficiency comparisons, and the inference procedures are tied to the Wald theory in Section~\ref{sec:theory}; see also Appendix~\ref{app:bootstrap}.

\item \textbf{Experiments compute resources.} \textbf{No.} The paper does not currently report wall-clock time, hardware, or aggregate compute usage for the simulation and benchmark runs.

\item \textbf{Code of ethics.} \textbf{Yes.} The paper is a methodological contribution in semisupervised statistical inference, and we are not aware of any aspect of the work that conflicts with the NeurIPS Code of Ethics.

\item \textbf{Broader impacts.} \textbf{N/A.} The paper develops estimation and inference methodology rather than a deployed decision system, though misuse of semisupervised estimators in high-stakes applications remains a general concern.

\item \textbf{Safeguards.} \textbf{N/A.} The paper does not introduce a system that generates content or interacts with users, and it does not release new high-risk artifacts requiring special safeguards.

\item \textbf{Licenses for existing assets.} \textbf{No.} The current draft cites the benchmark sources and related software ecosystems but does not yet include a consolidated license/version table for all reused assets.

\item \textbf{New assets.} \textbf{N/A.} The paper does not introduce a new dataset, benchmark, or foundation model.

\item \textbf{Crowdsourcing and research with human subjects.} \textbf{N/A.} The paper does not involve human-subject experiments or crowdsourcing.
\end{enumerate}
